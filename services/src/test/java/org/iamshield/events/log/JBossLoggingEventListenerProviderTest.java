 ...
 */
package org.iamshield.events.log;
...
import org.iamshield.services.resteasy.HttpRequestImpl;
...
public class JBossLoggingEventListenerProviderTest {
    ...
    private static void test(Map<String, String> config, AdminEvent adminEvent, boolean includeRepresentation, Consumer<String> assertMessage) {
        IAMShieldSession session = createSession(config);
        Logger logger = Logger.getLogger("org.iamshield.events");
    ...
    private static IAMShieldSession createSession(Map<String, String> config) {
        ...
        sessionFactory.init();
        IAMShieldSession session = new ResteasyIAMShieldSession(sessionFactory);
