package org.iamshield.services.error;

import static org.iamshield.services.resources.IAMShieldApplication.getSessionFactory;
...
@Provider
public class IAMShieldErrorHandler implements ExceptionMapper<Throwable> {

    private static final Logger logger = Logger.getLogger(IAMShieldErrorHandler.class);
    ...
    public Response toResponse(Throwable throwable) {
        IAMShieldSession session = IAMShieldSessionUtil.getIAMShieldSession();

        if (session == null) {
            // errors might be thrown when handling errors from JAX-RS before the session is available
            return IAMShieldModelUtils.runJobInTransactionWithResult(getSessionFactory(),
                    new IAMShieldSessionTaskWithResult<Response>() {
                        @Override
                        public Response run(IAMShieldSession session) {
    ...
    public static Response getResponse(IAMShieldSession session, Throwable throwable) {
        IAMShieldTransaction tx = session.getTransactionManager();
        tx.setRollbackOnly();
        ...
        try {
            ...
            if (theme == null) {
                String minimalHtml = "<!doctype html><html><head><meta charset=\"utf-8\"><title>Error</title></head><body><h1>Internal Server Error</h1><p>Unable to load LOGIN theme to render the error page. Please verify theme configuration. Showing a minimal fallback.</p></body></html>";
                return Response.status(responseStatus).type(MediaType.TEXT_HTML_UTF_8_TYPE).entity(minimalHtml).build();
            }
    ...
    private static RealmModel resolveRealm(IAMShieldSession session) {
    ...
    private static Map<String, Object> initAttributes(IAMShieldSession session, RealmModel realm, Theme theme, Locale locale, Response.Status responseStatus) throws IOException {
