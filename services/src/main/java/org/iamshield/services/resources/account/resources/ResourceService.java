...
import static org.iamshield.models.utils.ModelToRepresentation.toRepresentation;
...
public class ResourceService extends AbstractResourceService {
    ...
    private final org.iamshield.authorization.model.Resource resource;
    private final ResourceServer resourceServer;

    ResourceService(org.iamshield.authorization.model.Resource resource, IAMShieldSession session, UserModel user,
    ...
    public Response user(@QueryParam("value") String value) {
        try {
            ...
            return Response.ok(toRepresentation(provider.getIAMShieldSession(), provider.getRealm(), user)).build();
        ...
        for (Permission permission : permissions) {
            ...
            if (tickets.isEmpty()) {
                ...
                while (scopesIterator.hasNext()) {
                    org.iamshield.authorization.model.Scope scope = getScope(scopesIterator.next(), resourceServer);
    ...
    private void grantPermission(UserModel user, String scopeId) {
        org.iamshield.authorization.model.Scope scope = getScope(scopeId, resourceServer);
    ...
    private org.iamshield.authorization.model.Scope getScope(String scopeId, ResourceServer resourceServer) {
        org.iamshield.authorization.model.Scope scope = scopeStore.findByName(resourceServer, scopeId);
    ...
    private UserModel getUser(String requester) {
        UserProvider users = provider.getIAMShieldSession().users();
