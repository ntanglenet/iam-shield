...
import org.iamshield.AbstractOAuthClient;
...
public abstract class AbstractSecuredLocalService {
    ...
    public AbstractSecuredLocalService(IAMShieldSession session, ClientModel client) {
    ...
    public Response loginRedirect(@QueryParam("code") String code,
        ...
        try {
            ...
            IAMShieldUriBuilder redirect = IAMShieldUriBuilder.fromUri(getBaseRedirectUri());
            if (path != null) {
                redirect.path(path);
            }
            if (referrer != null) {
