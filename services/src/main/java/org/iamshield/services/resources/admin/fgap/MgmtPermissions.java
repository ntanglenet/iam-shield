...
class MgmtPermissions implements AdminPermissionEvaluator, AdminPermissionManagement, RealmsPermissionEvaluator {
    ...
    MgmtPermissions(IAMShieldSession session, RealmModel realm) {
        this.session = session;
        this.realm = realm;
        IAMShieldSessionFactory keycloakSessionFactory = session.getIAMShieldSessionFactory();
    ...
    MgmtPermissions(IAMShieldSession session, RealmModel realm, AdminAuth auth) {
        this(session, realm);
        this.auth = auth;
    ...
    MgmtPermissions(IAMShieldSession session, AdminAuth auth) {
        this.session = session;
        this.auth = auth;
        this.admin = auth.getUser();
        this.adminsRealm = auth.getRealm();
    ...
    private void initIdentity(IAMShieldSession session, AdminAuth auth) {
        ...
        this.identity = new IAMShieldIdentity(accessToken, session, adminsRealm);
    }

    MgmtPermissions(IAMShieldSession session, RealmModel adminsRealm, UserModel admin) {
        this.session = session;
        this.admin = admin;
    ...
    MgmtPermissions(IAMShieldSession session, RealmModel realm, RealmModel adminsRealm, UserModel admin) {
        this(session, realm);
        this.admin = admin;
        this.adminsRealm = adminsRealm;
