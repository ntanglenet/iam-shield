...
import org.iamshield.services.resources.account.AccountLoader;
...
import static org.iamshield.utils.MediaType.APPLICATION_JWT;
...
public class RealmsResource {
    ...
    public Object getProtocol(final @PathParam("realm") String name,
        ...
        LoginProtocolFactory factory = (LoginProtocolFactory)session.getIAMShieldSessionFactory().getProviderFactory(LoginProtocol.class, protocol);
        if(factory == null){
    ...
    public Response getRedirect(final @PathParam("realm") String realmName, final @PathParam("client_id") String clientId) {
        ...
        if (client.getRootUrl() != null && (client.getBaseUrl() == null || client.getBaseUrl().isEmpty())) {
            targetUri = IAMShieldUriBuilder.fromUri(client.getRootUrl()).build();
        } else {
            targetUri = IAMShieldUriBuilder.fromUri(ResolveRelative.resolveRelativeUri(session, client.getRootUrl(), client.getBaseUrl())).build();
        }
    ...
    private static void resolveRealmAndUpdateSession(IAMShieldSession session, String realmName) {
    ...
    public static Response getWellKnownResponse(IAMShieldSession session, String name, String alias, Logger logger) throws NotFoundException {
        resolveRealmAndUpdateSession(session, name);
        checkSsl(session, session.getContext().getRealm());

        WellKnownProviderFactory wellKnownProviderFactoryFound = session.getIAMShieldSessionFactory().getProviderFactoriesStream(WellKnownProvider.class)
    ...
    /**
     * A JAX-RS sub-resource locator that uses the {@link org.iamshield.services.resource.RealmResourceSPI} to resolve sub-resources instances given an <code>unknownPath</code>.
     *
    ...
    private static void checkSsl(IAMShieldSession session, RealmModel realm) {
