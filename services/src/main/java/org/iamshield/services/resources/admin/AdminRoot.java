...
import org.iamshield.services.resources.admin.info.ServerInfoAdminResource;
...
public class AdminRoot {
    ...
    public Response masterRealmAdminConsoleRedirect() {
        IAMShieldUriInfo adminUriInfo = session.getContext().getUri(UrlType.ADMIN);
    ...
    boolean shouldRedirect(IAMShieldUriInfo adminUriInfo) {
        if (!isAdminConsoleEnabled()) {
            return false;
        }
        IAMShieldUriInfo frontEndUriInfo = session.getContext().getUri();
    ...
    protected void resolveRealmAndUpdateSession(String name, IAMShieldSession session) {
    ...
    public static Theme getTheme(IAMShieldSession session, RealmModel realm) throws IOException {
        return session.theme().getTheme(Theme.Type.ADMIN);
    }

    public static Properties getMessages(IAMShieldSession session, RealmModel realm, String lang) {
        try {
    ...
    public static Properties getMessages(IAMShieldSession session, RealmModel realm, String lang, String... bundles) {
    ...
    private static Properties getMessages(IAMShieldSession session, RealmModel realm, String lang, String bundle) {
        try {
            Theme theme = getTheme(session, realm);
            Locale locale = lang != null ? Locale.forLanguageTag(lang) : Locale.ENGLISH;
