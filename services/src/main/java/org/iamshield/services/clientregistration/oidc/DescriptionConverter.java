...
import org.iamshield.protocol.oidc.OIDCClientSecretConfigWrapper;
...
import org.iamshield.protocol.oidc.utils.PairwiseSubMapperUtils;
...
import static org.iamshield.OAuth2Constants.AUTHORIZATION_CODE;
import static org.iamshield.OAuth2Constants.IMPLICIT;
import static org.iamshield.models.CibaConfig.OIDC_CIBA_GRANT_ENABLED;
import static org.iamshield.models.OAuth2DeviceConfig.OAUTH2_DEVICE_AUTHORIZATION_GRANT_ENABLED;
import static org.iamshield.protocol.oidc.utils.OIDCResponseType.CODE;
...
public class DescriptionConverter {
    ...
    public static ClientRepresentation toInternal(IAMShieldSession session, OIDCClientRepresentation clientOIDC) throws ClientRegistrationException {
        ...
        if ("none".equals(authMethod)) {
            ...
            if (authMethod == null) {
                clientAuthFactory = (ClientAuthenticatorFactory) session.getIAMShieldSessionFactory().getProviderFactory(ClientAuthenticator.class, IAMShieldModelUtils.getDefaultClientAuthenticatorType());
            } else {
    ...
    private static List<String> getSupportedAlgorithms(IAMShieldSession session, Class<? extends Provider> clazz, boolean includeNone) {
        Stream<String> supportedAlgorithms = session.getIAMShieldSessionFactory().getProviderFactoriesStream(clazz)
    ...
    private static boolean setPublicKey(OIDCClientRepresentation clientOIDC, ClientRepresentation clientRep) {
        ...
        if (clientOIDC.getJwks() != null) {
            ...
            String publicKeyPem = IAMShieldModelUtils.getPemFromKey(publicKey);
    ...
    public static OIDCClientRepresentation toExternalResponse(IAMShieldSession session, ClientRepresentation client, URI uri) {
        ...
        if ("none".equals(client.getClientAuthenticatorType())) {
            ...
            ClientAuthenticatorFactory clientAuth = (ClientAuthenticatorFactory) session.getIAMShieldSessionFactory().getProviderFactory(ClientAuthenticator.class, client.getClientAuthenticatorType());
