...
import org.iamshield.models.utils.DefaultClientScopes;
...
import org.iamshield.protocol.oidc.mappers.AudienceResolveProtocolMapper;
...
import org.iamshield.services.clientregistration.policy.DefaultClientRegistrationPolicies;
...
import org.iamshield.utils.SMTPUtil;
...
public class RealmManager {
    ...
    public RealmManager(IAMShieldSession session) {
    ...
    public RealmModel createRealm(String id, String name) {
        ...
        IAMShieldModelUtils.setupDefaultRole(realm, Constants.DEFAULT_ROLES_ROLE_PREFIX + "-" + name.toLowerCase());
    ...
    private void setupOfflineTokens(RealmModel realm, RealmRepresentation realmRep) {
        RoleModel offlineRole = IAMShieldModelUtils.setupOfflineRole(realm);
    ...
    protected void setupAdminConsole(RealmModel realm) {
        ...
        if (adminConsole == null) adminConsole = IAMShieldModelUtils.createPublicClient(realm, Constants.ADMIN_CONSOLE_CLIENT_ID);
    ...
    public void setupAdminCli(RealmModel realm) {
        ...
        if (adminCli == null) {
            adminCli = IAMShieldModelUtils.createPublicClient(realm, Constants.ADMIN_CLI_CLIENT_ID);
    ...
    public void updateRealmEventsConfig(RealmEventsConfigRepresentation rep, RealmModel realm) {
        ...
        if (rep.getEventsListeners() != null) {
            for (String el : rep.getEventsListeners()) {
                EventListenerProviderFactory elpf = (EventListenerProviderFactory) session.getIAMShieldSessionFactory().getProviderFactory(EventListenerProvider.class, el);
                if (elpf == null) {
    ...
    public void setupMasterAdminManagement(RealmModel realm) {
        ...
        ClientModel masterApp = adminRealm.getClientByClientId(IAMShieldModelUtils.getMasterRealmAdminManagementClientId(realm.getName()));
        if (masterApp == null) {
    ...
    private void createMasterAdminManagement(RealmModel realm) {
        ...
        ClientModel realmAdminApp = IAMShieldModelUtils.createManagementClient(adminRealm, IAMShieldModelUtils.getMasterRealmAdminManagementClientId(realm.getName()));
    ...
    private void setupRealmAdminManagement(RealmModel realm) {
        ...
        if (realmAdminClient == null) {
            realmAdminClient = IAMShieldModelUtils.createManagementClient(realm, realmAdminClientId);
    ...
    private void setupAccountManagement(RealmModel realm) {
        ...
        if (accountClient == null) {
            accountClient = IAMShieldModelUtils.createPublicClient(realm, Constants.ACCOUNT_MANAGEMENT_CLIENT_ID);
            ...
            IAMShieldModelUtils.setupDeleteAccount(accountClient);
            ...
            if (accountConsoleClient == null) {
                accountConsoleClient = IAMShieldModelUtils.createPublicClient(realm, Constants.ACCOUNT_CONSOLE_CLIENT_ID);
    ...
    public void setupBrokerService(RealmModel realm) {
        ...
        if (client == null) {
            client = IAMShieldModelUtils.createManagementClient(realm, Constants.BROKER_SERVICE_CLIENT_ID);
    ...
    public RealmModel importRealm(RealmRepresentation rep, Runnable userImport) {
        ...
        try {
            ...
            if (rep.getDefaultRole() == null) {
                IAMShieldModelUtils.setupDefaultRole(realm, determineDefaultRoleName(rep));
            } else {
            ...
            if (!hasRealmRole(rep, AccountRoles.DELETE_ACCOUNT)) {
                IAMShieldModelUtils.setupDeleteAccount(realm.getClientByClientId(Constants.ACCOUNT_MANAGEMENT_CLIENT_ID));
            }
    ...
    private boolean hasRealmAdminManagementClient(RealmRepresentation rep) {
        String realmAdminClientId =  Config.getAdminRealm().equals(rep.getRealm()) ?  IAMShieldModelUtils.getMasterRealmAdminManagementClientId(rep.getRealm()) : getRealmAdminClientId(rep);
    ...
    private void setupAuthorizationServices(RealmModel realm) {
        IAMShieldModelUtils.setupAuthorizationServices(realm);
    }
    ...
    private void fireRealmPostCreate(RealmModel realm) {
        session.getIAMShieldSessionFactory().publish(new RealmModel.RealmPostCreateEvent() {
            @Override
    ...
    public void setDefaultsForNewRealm(RealmModel realm) {
        // setup defaults
        setupRealmDefaults(realm);
        IAMShieldModelUtils.setupDefaultRole(realm, Constants.DEFAULT_ROLES_ROLE_PREFIX + "-" + realm.getName().toLowerCase());
        setupRealmAdminManagement(realm);
        setupAccountManagement(realm);
        setupBrokerService(realm);
        setupAdminConsole(realm);
