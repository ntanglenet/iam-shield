...
public class DefaultBlockingBruteForceProtector extends DefaultBruteForceProtector {
    ...
    DefaultBlockingBruteForceProtector(IAMShieldSessionFactory factory) {
        super(factory);
    }
    ...
    private boolean isLoginInProgress(IAMShieldSession session, UserModel user) {
    ...
    private boolean tryEnlistBlockingTransactionOrSameThread(IAMShieldSession session, UserModel user) {
    ...
    private void enlistRemoval(IAMShieldSession session, String userId) {
        session.getTransactionManager().enlistAfterCompletion(new AbstractIAMShieldTransaction() {
            @Override
            protected void commitImpl() {
