...
class CodeGenerateUtil {
    ...
    interface ClientSessionParser<CS extends CommonClientSessionModel> {
        ...
        CS parseSession(String code, String tabId, IAMShieldSession session, RealmModel realm, ClientModel client, EventBuilder event);

        String retrieveCode(IAMShieldSession session, CS clientSession);

        void removeExpiredSession(IAMShieldSession session, CS clientSession);

        boolean verifyCode(IAMShieldSession session, String code, CS clientSession);

        boolean isExpired(IAMShieldSession session, String code, CS clientSession);
    ...
    private static class AuthenticationSessionModelParser implements ClientSessionParser<AuthenticationSessionModel> {
        ...
        @Override
        public AuthenticationSessionModel parseSession(String code, String tabId, IAMShieldSession session, RealmModel realm, ClientModel client, EventBuilder event) {
        ...
        @Override
        public String retrieveCode(IAMShieldSession session, AuthenticationSessionModel authSession) {
            ...
            if (nextCode == null) {
                ...
                session.getTransactionManager().enlist(new RollbackDrivenTransaction(session.getIAMShieldSessionFactory(), currentSession -> {
        ...
        @Override
        public void removeExpiredSession(IAMShieldSession session, AuthenticationSessionModel clientSession) {
            new AuthenticationSessionManager(session).removeAuthenticationSession(clientSession.getRealm(), clientSession, true);
        }


        @Override
        public boolean verifyCode(IAMShieldSession session, String code, AuthenticationSessionModel authSession) {
        ...
        @Override
        public boolean isExpired(IAMShieldSession session, String code, AuthenticationSessionModel clientSession) {
            return false;
        }
    ...
    /**
     * A {@link IAMShieldTransaction} that runs a task only when {@link #rollback()} is called.
     */
    private static class RollbackDrivenTransaction implements IAMShieldTransaction {

        private final IAMShieldSessionFactory factory;
        private final IAMShieldSessionTask task;

        RollbackDrivenTransaction(final IAMShieldSessionFactory factory, final IAMShieldSessionTask task) {
        ...
        public void rollback() {
            IAMShieldModelUtils.runJobInTransaction(this.factory, this.task);
        }
