...
import org.iamshield.http.FormPartValue;
...
import static org.iamshield.models.UserModel.DISABLED_REASON;
...
public class DefaultBruteForceProtector implements BruteForceProtector {
    ...
    protected IAMShieldSessionFactory factory;

    public DefaultBruteForceProtector(IAMShieldSessionFactory factory) {
        this.factory = factory;
    }

    protected void failure(IAMShieldSession session, RealmModel realm, String userId, String remoteAddr, long failureTime) {
    ...
    protected UserLoginFailureModel getUserFailureModel(IAMShieldSession session, RealmModel realm, String userId) {
        if (realm == null) return null;
        return session.loginFailures().getUserLoginFailure(realm, userId);
    }

    protected void sendEvent(IAMShieldSession session, RealmModel realm, UserLoginFailureModel userLoginFailure, EventType type) {
    ...
    protected void success(IAMShieldSession session, RealmModel realm, String userId) {
        UserLoginFailureModel userLoginFailure = getUserFailureModel(session, realm, userId);
    ...
    protected void processLogin(RealmModel realm, UserModel user, ClientConnection clientConnection, UriInfo uriInfo, boolean success) {
        ExecutorService executor = IAMShieldModelUtils.runJobInTransactionWithResult(factory, session -> {
        ...
        executor.execute(() -> IAMShieldModelUtils.runJobInTransaction(factory, s -> {
    ...
    @Override
    public boolean isTemporarilyDisabled(IAMShieldSession session, RealmModel realm, UserModel user) {
    ...
    @Override
    public boolean isPermanentlyLockedOut(IAMShieldSession session, RealmModel realm, UserModel user) {
    ...
    @Override
    public void cleanUpPermanentLockout(IAMShieldSession session, RealmModel realm, UserModel user) {
