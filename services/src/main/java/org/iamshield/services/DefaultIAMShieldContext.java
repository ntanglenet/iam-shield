...
import org.iamshield.logging.MappedDiagnosticContextProvider;
 ...
 */
public abstract class DefaultIAMShieldContext implements IAMShieldContext {
    ...
    private Map<UrlType, IAMShieldUriInfo> uriInfo;
    ...
    public DefaultIAMShieldContext(IAMShieldSession session) {
    ...
    @Override
    public IAMShieldUriInfo getUri(UrlType type) {
        if (uriInfo == null || !uriInfo.containsKey(type)) {
            if (uriInfo == null) {
                uriInfo = new HashMap<>();
            }

            uriInfo.put(type, new IAMShieldUriInfo(session, type, getHttpRequest().getUri()));
        }
        return uriInfo.get(type);
    }

    @Override
    public IAMShieldUriInfo getUri() {
