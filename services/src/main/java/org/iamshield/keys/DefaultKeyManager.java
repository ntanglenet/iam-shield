...
public class DefaultKeyManager implements KeyManager {
    ...
    public DefaultKeyManager(IAMShieldSession session) {
    ...
    public KeyWrapper getActiveKey(RealmModel realm, KeyUse use, String algorithm) {
        ...
        Optional<KeyProviderFactory> keyProviderFactory = session.getIAMShieldSessionFactory()
    ...
    private List<KeyProvider> getProviders(RealmModel realm) {
        ...
        if (providers == null) {
            providers = realm.getComponentsStream(realm.getId(), KeyProvider.class.getName())
                    ...
                    .map(c -> {
                        try {
                            ProviderFactory<KeyProvider> f = session.getIAMShieldSessionFactory().getProviderFactory(KeyProvider.class, c.getProviderId());
