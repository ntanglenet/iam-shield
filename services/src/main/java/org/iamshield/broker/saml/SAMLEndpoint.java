...
package org.iamshield.broker.saml;
...
import org.iamshield.dom.saml.v2.assertion.AuthnStatementType;
...
import org.iamshield.dom.saml.v2.assertion.SubjectConfirmationType;
...
import org.iamshield.protocol.saml.SamlService;
import org.iamshield.protocol.saml.SamlSessionUtils;
import org.iamshield.protocol.saml.preprocessor.SamlAuthenticationPreprocessor;
import org.iamshield.protocol.saml.SAMLDecryptionKeysLocator;
...
import org.iamshield.saml.processing.core.saml.v2.constants.X500SAMLProfileConstants;
import org.iamshield.saml.processing.core.saml.v2.util.ArtifactResponseUtil;
import org.iamshield.saml.processing.core.saml.v2.util.AssertionUtil;
import org.iamshield.saml.processing.core.util.XMLEncryptionUtil;
...
import org.iamshield.protocol.saml.SamlMetadataKeyLocator;
import org.iamshield.protocol.saml.SamlMetadataPublicKeyLoader;
import org.iamshield.protocol.saml.SamlPrincipalType;
...
import org.iamshield.saml.validators.ConditionsValidator;
...
public class SAMLEndpoint {
    ...
    public SAMLEndpoint(IAMShieldSession session, SAMLIdentityProvider provider, SAMLIdentityProviderConfig config, IdentityProvider.AuthenticationCallback callback, DestinationValidator destinationValidator) {
    ...
    protected abstract class Binding {
        ...
        protected Response logoutRequest(LogoutRequestType request, String relayState) {
            ...
            if (config.isWantAuthnRequestsSigned()) {
                ...
                if (! postBinding && config.isAddExtensionsElementWithKeyInfo()) {    // Only include extension if REDIRECT binding and signing whole SAML protocol message
                    builder.addExtension(new IAMShieldKeySamlExtensionGenerator(keyName));
                }
            }
            try {
                if (postBinding) {
        ...
        private AuthenticationSessionModel samlIdpInitiatedSSO(final String clientUrlName) {
            ...
            LoginProtocolFactory factory = (LoginProtocolFactory) session.getIAMShieldSessionFactory().getProviderFactory(LoginProtocol.class, SamlProtocol.LOGIN_PROTOCOL);
