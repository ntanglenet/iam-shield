...
import org.iamshield.broker.provider.util.SimpleHttp;
...
import org.iamshield.dom.saml.v2.metadata.EndpointType;
...
import org.iamshield.dom.saml.v2.metadata.KeyTypes;
...
import org.iamshield.protocol.saml.mappers.SamlMetadataDescriptorUpdater;
import org.iamshield.protocol.saml.preprocessor.SamlAuthenticationPreprocessor;
import org.iamshield.protocol.saml.SAMLEncryptionAlgorithms;
import org.iamshield.protocol.saml.profile.util.Soap;
import org.iamshield.saml.SAML2ArtifactResolveRequestBuilder;
import org.iamshield.saml.SAML2AuthnRequestBuilder;
import org.iamshield.saml.SAML2LogoutRequestBuilder;
import org.iamshield.saml.SAML2NameIDPolicyBuilder;
import org.iamshield.saml.SAML2RequestedAuthnContextBuilder;
import org.iamshield.saml.SPMetadataDescriptor;
import org.iamshield.saml.SamlProtocolExtensionsAwareBuilder.NodeGenerator;
...
public class SAMLIdentityProvider extends AbstractIdentityProvider<SAMLIdentityProviderConfig> {
    ...
    public SAMLIdentityProvider(IAMShieldSession session, SAMLIdentityProviderConfig config, DestinationValidator destinationValidator) {
        ...
        try {
            ...
            if (getConfig().isWantAuthnRequestsSigned()) {
                ...
                if (! postBinding && getConfig().isAddExtensionsElementWithKeyInfo()) {    // Only include extension if REDIRECT binding and signing whole SAML protocol message
                    authnRequestBuilder.addExtension(new IAMShieldKeySamlExtensionGenerator(keyName));
                }
            }
    ...
    private JaxrsSAML2BindingBuilder buildLogoutBinding(IAMShieldSession session, UserSessionModel userSession, RealmModel realm) {
        JaxrsSAML2BindingBuilder binding = new JaxrsSAML2BindingBuilder(session)
    ...
    public Response export(UriInfo uriInfo, RealmModel realm, String format) {
        try
        {
            ...
            session.identityProviders().getMappersByAliasStream(getConfig().getAlias())
                .forEach(mapper -> {
                    IdentityProviderMapper target = (IdentityProviderMapper) session.getIAMShieldSessionFactory().getProviderFactory(IdentityProviderMapper.class, mapper.getIdentityProviderMapper());
    ...
    public SAMLDocumentHolder resolveArtifact(IAMShieldSession session, UriInfo uriInfo, RealmModel realm, String relayState, String samlArt) {
    ...
    private JaxrsSAML2BindingBuilder buildArtifactResolveBinding(IAMShieldSession session, String relayState, RealmModel realm) {
