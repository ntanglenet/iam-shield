 ...
 * @version $Revision: 1 $
 */
public class ExternalIAMShieldRoleToRoleMapper extends AbstractClaimToRoleMapper {

    public static final String[] COMPATIBLE_PROVIDERS = {IAMShieldOIDCIdentityProviderFactory.PROVIDER_ID};
    ...
    protected boolean applies(IdentityProviderMapperModel mapperModel, BrokeredIdentityContext context) {
        JsonWebToken token = (JsonWebToken)context.getContextData().get(IAMShieldOIDCIdentityProvider.VALIDATED_ACCESS_TOKEN);
        String[] parseRole = IAMShieldModelUtils.parseRole(mapperModel.getConfig().get(EXTERNAL_ROLE));
