...
import org.iamshield.protocol.oidc.mappers.AcrProtocolMapper;
import org.iamshield.protocol.oidc.mappers.AddressMapper;
import org.iamshield.protocol.oidc.mappers.AllowedWebOriginsProtocolMapper;
import org.iamshield.protocol.oidc.mappers.AudienceResolveProtocolMapper;
import org.iamshield.protocol.oidc.mappers.FullNameMapper;
import org.iamshield.organization.protocol.mappers.oidc.OrganizationMembershipMapper;
import org.iamshield.protocol.oidc.mappers.UserAttributeMapper;
import org.iamshield.protocol.oidc.mappers.UserClientRoleMappingMapper;
import org.iamshield.protocol.oidc.mappers.UserPropertyMapper;
import org.iamshield.protocol.oidc.mappers.UserRealmRoleMappingMapper;
import org.iamshield.protocol.oidc.mappers.UserSessionNoteMapper;
import org.iamshield.protocol.oidc.mappers.SubMapper;
...
import static org.iamshield.models.ImpersonationSessionNote.IMPERSONATOR_ID;
import static org.iamshield.models.ImpersonationSessionNote.IMPERSONATOR_USERNAME;
import static org.iamshield.protocol.oidc.OIDCProviderConfig.DEFAULT_ADDITIONAL_REQ_PARAMS_FAIL_FAST;
import static org.iamshield.protocol.oidc.OIDCProviderConfig.DEFAULT_ADDITIONAL_REQ_PARAMS_MAX_NUMBER;
import static org.iamshield.protocol.oidc.OIDCProviderConfig.DEFAULT_ADDITIONAL_REQ_PARAMS_MAX_OVERALL_SIZE;
import static org.iamshield.protocol.oidc.OIDCProviderConfig.DEFAULT_ADDITIONAL_REQ_PARAMS_MAX_SIZE;
import static org.iamshield.protocol.oidc.OIDCProviderConfig.DEFAULT_REQ_PARAMS_DEFAULT_MAX_SIZE;
...
public class OIDCLoginProtocolFactory extends AbstractLoginProtocolFactory {
    ...
    @Override
    public LoginProtocol create(IAMShieldSession session) {
    ...
    protected void createDefaultClientScopesImpl(RealmModel newRealm) {
        ...
        if (offlineRole != null) {
            ClientScopeModel offlineAccessScope = IAMShieldModelUtils.getClientScopeByName(newRealm, OAuth2Constants.OFFLINE_ACCESS);
    ...
    public ClientScopeModel addRolesClientScope(RealmModel newRealm) {
        ClientScopeModel rolesScope = IAMShieldModelUtils.getClientScopeByName(newRealm, ROLES_SCOPE);
        if (rolesScope == null) {
    ...
    public ClientScopeModel addWebOriginsClientScope(RealmModel newRealm) {
        ClientScopeModel originsScope = IAMShieldModelUtils.getClientScopeByName(newRealm, WEB_ORIGINS_SCOPE);
    ...
    public ClientScopeModel addMicroprofileJWTClientScope(RealmModel newRealm) {
        ClientScopeModel microprofileScope = IAMShieldModelUtils.getClientScopeByName(newRealm, MICROPROFILE_JWT_SCOPE);
    ...
    public ClientScopeModel addAcrClientScope(RealmModel newRealm) {
        if (Profile.isFeatureEnabled(Profile.Feature.STEP_UP_AUTHENTICATION)) {
            ClientScopeModel acrScope = IAMShieldModelUtils.getClientScopeByName(newRealm, ACR_SCOPE);
            if (acrScope == null) {
    ...
    public ClientScopeModel addBasicClientScope(RealmModel newRealm) {
        ClientScopeModel basicScope = IAMShieldModelUtils.getClientScopeByName(newRealm, BASIC_SCOPE);
        if (basicScope == null) {
    ...
    public ClientScopeModel addServiceAccountClientScope(RealmModel newRealm) {
        ClientScopeModel serviceAccountScope = IAMShieldModelUtils.getClientScopeByName(newRealm, ServiceAccountConstants.SERVICE_ACCOUNT_SCOPE);
    ...
    @Override
    public Object createProtocolEndpoint(IAMShieldSession session, EventBuilder event) {
    ...
    public void setupClientDefaults(ClientRepresentation rep, ClientModel newClient) {
        ...
        if (rep.isBearerOnly() == null
            ...
            if (newClient.getSecret() == null) {
                IAMShieldModelUtils.generateSecret(newClient);
            }
        }
