...
import org.iamshield.TokenIdGenerator;
...
import org.iamshield.constants.AdapterConstants;
...
import org.iamshield.services.clientpolicy.context.ImplicitHybridTokenResponse;
...
import static org.iamshield.protocol.oidc.grants.device.DeviceGrantType.approveOAuth2DeviceAuthorization;
import static org.iamshield.protocol.oidc.grants.device.DeviceGrantType.denyOAuth2DeviceAuthorization;
...
public class OIDCLoginProtocol implements LoginProtocol {
    ...
    public OIDCLoginProtocol(IAMShieldSession session, RealmModel realm, UriInfo uriInfo, HttpHeaders headers, EventBuilder event) {
    ...
    @Override
    public OIDCLoginProtocol setSession(IAMShieldSession session) {
        this.session = session;
        return this;
    }
    ...
    public Response authenticated(AuthenticationSessionModel authSession, UserSessionModel userSession, ClientSessionContext clientSessionCtx) {
        ...
        if (responseType.isImplicitOrHybridFlow()) {
            org.iamshield.protocol.oidc.TokenManager tokenManager = new org.iamshield.protocol.oidc.TokenManager();
            org.iamshield.protocol.oidc.TokenManager.AccessTokenResponseBuilder responseBuilder = tokenManager.responseBuilder(realm, clientSession.getClient(), event, session, userSession, clientSessionCtx)
                .generateAccessToken();
