 ...
 */
package org.iamshield.protocol.oidc.grants.ciba.resolvers;
...
public class DefaultCIBALoginUserResolver implements CIBALoginUserResolver {
    ...
    public DefaultCIBALoginUserResolver(IAMShieldSession session) {
        this.session = session;
    }

    @Override
    public UserModel getUserFromLoginHint(String loginHint) {
        return IAMShieldModelUtils.findUserByNameOrEmail(session, session.getContext().getRealm(), loginHint);
    }
    ...
    public UserModel getUserFromInfoUsedByAuthentication(String info) {
        return IAMShieldModelUtils.findUserByNameOrEmail(session, session.getContext().getRealm(), info);
    }
