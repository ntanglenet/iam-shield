...
import org.iamshield.protocol.ClientData;
...
import org.iamshield.protocol.saml.mappers.NameIdMapperHelper;
import org.iamshield.protocol.saml.mappers.SAMLAttributeStatementMapper;
import org.iamshield.protocol.saml.mappers.SAMLLoginResponseMapper;
import org.iamshield.protocol.saml.mappers.SAMLNameIdMapper;
import org.iamshield.protocol.saml.mappers.SAMLRoleListMapper;
...
import org.iamshield.saml.SAML2ErrorResponseBuilder;
...
import org.iamshield.services.managers.ResourceAdminManager;
...
public class SamlProtocol implements LoginProtocol {
    ...
    @Override
    public SamlProtocol setSession(IAMShieldSession session) {
    ...
    public AttributeStatementType populateAttributeStatements(List<ProtocolMapperProcessor<SAMLAttributeStatementMapper>> attributeStatementMappers, IAMShieldSession session, UserSessionModel userSession,
    ...
    public ResponseType transformLoginResponse(List<ProtocolMapperProcessor<SAMLLoginResponseMapper>> mappers, ResponseType response,
            IAMShieldSession session, UserSessionModel userSession, ClientSessionContext clientSessionCtx) {
    ...
    public void populateRoles(ProtocolMapperProcessor<SAMLRoleListMapper> roleListMapper, IAMShieldSession session, UserSessionModel userSession,
    ...
    protected String getSAMLNameId(
            List<ProtocolMapperProcessor<SAMLNameIdMapper>> samlNameIdMappers, String nameIdFormat, IAMShieldSession session,
    ...
    public static String getLogoutServiceUrl(IAMShieldSession session, ClientModel client, String bindingType, boolean backChannelLogout) {
        ...
        try {
            ...
            if (!SAML_POST_BINDING.equals(logoutBindingType)) {
                if (samlClient.requiresRealmSignature() && samlClient.addExtensionsElementWithKeyInfo()) {
                    ...
                    String keyName = samlClient.getXmlSigKeyInfoKeyNameTransformer().getKeyName(keys.getKid(), keys.getCertificate());
                    extensions = new NodeGenerator[]{new IAMShieldKeySamlExtensionGenerator(keyName)};
                }
            }
    ...
    private Response artifactRedirect(String redirectUri, String artifact, String relayState) {
        IAMShieldUriBuilder builder = IAMShieldUriBuilder.fromUri(redirectUri)
                .replaceQuery(null)
