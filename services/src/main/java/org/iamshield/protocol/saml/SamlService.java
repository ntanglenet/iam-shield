...
import org.iamshield.broker.saml.SAMLDataMarshaller;
...
import org.iamshield.crypto.KeyStatus;
import org.iamshield.crypto.KeyUse;
import org.iamshield.crypto.KeyWrapper;
import org.iamshield.dom.saml.v2.SAML2Object;
import org.iamshield.dom.saml.v2.assertion.BaseIDAbstractType;
import org.iamshield.dom.saml.v2.assertion.NameIDType;
import org.iamshield.dom.saml.v2.assertion.SubjectType;
...
import org.iamshield.dom.saml.v2.protocol.LogoutRequestType;
import org.iamshield.dom.saml.v2.protocol.NameIDPolicyType;
import org.iamshield.dom.saml.v2.protocol.RequestAbstractType;
import org.iamshield.dom.saml.v2.protocol.ResponseType;
import org.iamshield.dom.saml.v2.protocol.StatusResponseType;
...
import org.iamshield.executors.ExecutorsProvider;
...
import org.iamshield.protocol.saml.profile.ecp.SamlEcpProfileService;
import org.iamshield.protocol.saml.profile.util.Soap;
import org.iamshield.protocol.saml.util.ArtifactBindingUtils;
import org.iamshield.rotation.HardcodedKeyLocator;
import org.iamshield.rotation.KeyLocator;
import org.iamshield.saml.BaseSAML2BindingBuilder;
import org.iamshield.saml.SAML2LogoutResponseBuilder;
import org.iamshield.saml.SAML2NameIDBuilder;
import org.iamshield.saml.SAMLRequestParser;
import org.iamshield.saml.SignatureAlgorithm;
import org.iamshield.saml.common.constants.GeneralConstants;
import org.iamshield.saml.common.constants.JBossSAMLConstants;
...
import org.iamshield.saml.common.util.StaxUtil;
import org.iamshield.saml.processing.api.saml.v2.request.SAML2Request;
import org.iamshield.saml.processing.core.saml.v2.common.IDGenerator;
import org.iamshield.saml.processing.core.saml.v2.common.SAMLDocumentHolder;
import org.iamshield.saml.processing.core.saml.v2.util.XMLTimeUtil;
import org.iamshield.saml.processing.core.saml.v2.writers.SAMLRequestWriter;
import org.iamshield.saml.processing.core.util.IAMShieldKeySamlExtensionGenerator;
import org.iamshield.saml.processing.web.util.PostBindingUtil;
import org.iamshield.saml.processing.web.util.RedirectBindingUtil;
import org.iamshield.saml.validators.DestinationValidator;
...
import org.iamshield.services.scheduled.ScheduledTaskRunner;
...
import org.iamshield.transaction.AsyncResponseTransaction;
...
import static org.iamshield.common.util.StackUtil.getShortStackTrace;
...
public class SamlService extends AuthorizationEndpointBase {
    ...
    public SamlService(IAMShieldSession session, EventBuilder event, DestinationValidator destinationValidator) {
        super(session, event);
    ...
    public abstract class BindingProtocol {
        ...
        protected abstract Response error(IAMShieldSession session, AuthenticationSessionModel authenticationSession, Response.Status status, String message, Object... parameters);
        ...
        protected void handleArtifact(AsyncResponse asyncResponse, String artifact, String relayState) {
            ...
            try {
                ...
                ScheduledTaskRunner task = new ScheduledTaskRunner(session.getIAMShieldSessionFactory(), artifactResolutionRunnable);
        ...
        protected Response logoutRequest(LogoutRequestType logoutRequest, ClientModel client, String relayState) {
            ...
            if (samlClient.requiresRealmSignature()) {
                ...
                if (! postBinding && samlClient.addExtensionsElementWithKeyInfo()) {    // Only include extension if REDIRECT binding and signing whole SAML protocol message
                    builder.addExtension(new IAMShieldKeySamlExtensionGenerator(keys.getKid()));
                }
            }
            try {
                if (postBinding) {
         ...
         *
         * @param session a reference to the {@link IAMShieldSession}.
         * @return the constructed {@link URI}.
         */
        protected URI getExpectedDestinationUri(final IAMShieldSession session) {
    ...
    protected class PostBindingProtocol extends BindingProtocol {
        ...
        @Override
        protected Response error(IAMShieldSession session, AuthenticationSessionModel authenticationSession, Response.Status status, String message, Object... parameters) {
    ...
    protected class RedirectBindingProtocol extends BindingProtocol {
        ...
        @Override
        protected boolean containsUnencryptedSignature(SAMLDocumentHolder documentHolder) {
            IAMShieldUriInfo uriInformation = session.getContext().getUri();
    ...
    public static String getIDPMetadataDescriptor(UriInfo uriInfo, IAMShieldSession session, RealmModel realm) {
        try {
     ...
     */
    public AuthenticationSessionModel getOrCreateLoginSessionForIdpInitiatedSso(IAMShieldSession session, RealmModel realm, ClientModel client, String relayState) {
        ...
        public void run(IAMShieldSession session){
            ...
            try (CloseableHttpResponse result = httpClient.execute(httpPost)) {
                try {
                    ...
                    LoginProtocolFactory factory = (LoginProtocolFactory) session.getIAMShieldSessionFactory().getProviderFactory(LoginProtocol.class, "saml");
                    if (factory == null) {
