...
import static org.iamshield.models.workflow.Workflow.RECURRING_KEY;
import static org.iamshield.models.workflow.Workflow.SCHEDULED_KEY;
import static org.iamshield.models.workflow.WorkflowStep.AFTER_KEY;
...
public class WorkflowsManager {
    ...
    public WorkflowsManager(IAMShieldSession session) {
        this.session = session;
        this.workflowStateProvider = session.getIAMShieldSessionFactory().getProviderFactory(WorkflowStateProvider.class).create(session);
    }
    ...
    private WorkflowProvider getWorkflowProvider(Workflow workflow) {
        ComponentFactory<?, ?> factory = (ComponentFactory<?, ?>) session.getIAMShieldSessionFactory()
    ...
    public WorkflowStepProvider getStepProvider(WorkflowStep step) {
        ComponentFactory<?, ?> stepFactory = (ComponentFactory<?, ?>) session.getIAMShieldSessionFactory()
                            ...
                            if (provider.activateOnEvent(event)) {
                                if (workflow.isScheduled()) {
                                    ...
                                    log.debugf("Running all steps of workflow %s for resource %s based on event %s",
                                            workflow.getId(), event.getResourceId(), event.getOperation());
                                    IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), session.getContext(), s ->
