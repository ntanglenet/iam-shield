...
public class RolesPartialImport implements PartialImport<RolesRepresentation> {
    ...
    @Override
    public void prepare(PartialImportRepresentation rep, RealmModel realm, IAMShieldSession session) {
        prepareRealmRoles(rep, realm, session);
        prepareClientRoles(rep, realm, session);
    }

    private void prepareRealmRoles(PartialImportRepresentation rep, RealmModel realm, IAMShieldSession session) {
    ...
    private void prepareClientRoles(PartialImportRepresentation rep, RealmModel realm, IAMShieldSession session) {
    ...
    @Override
    public void removeOverwrites(RealmModel realm, IAMShieldSession session) {
        deleteClientRoleOverwrites(realm);
        deleteRealmRoleOverwrites(realm, session);
    }

    @Override
    public PartialImportResults doImport(PartialImportRepresentation rep, RealmModel realm, IAMShieldSession session) {
    ...
    private void setUniqueIds(List<RoleRepresentation> realmRoles) {
        for (RoleRepresentation realmRole : realmRoles) {
            realmRole.setId(IAMShieldModelUtils.generateId());
        }
    }
    ...
    private void setUniqueIds(Map<String, List<RoleRepresentation>> clientRoles) {
        for (List<RoleRepresentation> roleRepresentations : clientRoles.values()) {
            for (RoleRepresentation clientRole : roleRepresentations) {
                clientRole.setId(IAMShieldModelUtils.generateId());
            }
        }
    }
    ...
    private void deleteRealmRoleOverwrites(RealmModel realm, IAMShieldSession session) {
    ...
    private void addResultsForOverwrittenRealmRoles(PartialImportResults results, RealmModel realm, IAMShieldSession session) {
        if (isEmpty(realmRolesToOverwrite)) return;
