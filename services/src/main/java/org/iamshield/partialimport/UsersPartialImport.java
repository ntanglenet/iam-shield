...
public class UsersPartialImport extends AbstractPartialImport<UserRepresentation> {
    ...
    @Override
    public String getModelId(RealmModel realm, IAMShieldSession session, UserRepresentation user) {
    ...
    @Override
    public boolean exists(RealmModel realm, IAMShieldSession session, UserRepresentation user) {
        return userNameExists(realm, session, user) || userEmailExists(realm, session, user);
    }

    private boolean userNameExists(RealmModel realm, IAMShieldSession session, UserRepresentation user) {
        return session.users().getUserByUsername(realm, user.getUsername()) != null;
    }

    private boolean userEmailExists(RealmModel realm, IAMShieldSession session, UserRepresentation user) {
    ...
    @Override
    public void remove(RealmModel realm, IAMShieldSession session, UserRepresentation user) {
    ...
    @Override
    public void create(RealmModel realm, IAMShieldSession session, UserRepresentation user) {
        if (user.getId() == null) {
            user.setId(IAMShieldModelUtils.generateId());
        }
