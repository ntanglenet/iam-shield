...
public class ConditionalUserAttributeValue implements ConditionalAuthenticator {
    ...
    public boolean matchCondition(AuthenticationFlowContext context) {
        ...
        if (!result && includeGroupAttributes) {
            result = IAMShieldModelUtils.resolveAttribute(user, attributeName, true).stream().anyMatch(attr -> regexOutput ? Pattern.compile(attributeValue).matcher(attr).matches() : Objects.equals(attr, attributeValue));
        }
