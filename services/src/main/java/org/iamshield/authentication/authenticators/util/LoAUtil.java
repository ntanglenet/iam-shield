...
package org.iamshield.authentication.authenticators.util;
...
import static org.iamshield.models.Constants.NO_LOA;
...
public class LoAUtil {
     ...
     */
    public static Map<String, Integer> getCredentialTypesToLoAMap(IAMShieldSession session, RealmModel realm, AuthenticationFlowModel topFlow) {
    ...
    private static void fillCredentialsToLoAMap(IAMShieldSession session, RealmModel realm, AuthenticationFlowModel authFlow, Set<String> availableCredentialTypes, AtomicReference<Integer> currentLevel, Map<String, Integer> result) {
        realm.getAuthenticationExecutionsStream(authFlow.getId()).forEachOrdered(execution -> {
            if (execution.isAuthenticatorFlow()) {
                ...
                if (ConditionalLoaAuthenticatorFactory.PROVIDER_ID.equals(execution.getAuthenticator())) {
                    ...
                    AuthenticatorFactory factory = (AuthenticatorFactory) session.getIAMShieldSessionFactory().getProviderFactory(Authenticator.class, execution.getAuthenticator());
