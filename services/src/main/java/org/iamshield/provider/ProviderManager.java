...
public class ProviderManager {
    ...
    public ProviderManager(IAMShieldDeploymentInfo info, ClassLoader baseClassLoader, String... resources) {
        this.info = info;
        ...
        if (resources != null) {
            for (String r : resources) {
                ...
                for (ProviderLoaderFactory f : factories) {
                    if (f.supports(type)) {
                        IAMShieldDeploymentInfo resourceInfo = IAMShieldDeploymentInfo.create().services();
    ...
    public ProviderManager(IAMShieldDeploymentInfo info, ClassLoader baseClassLoader, Collection<ProviderLoader> additionalProviderLoaders) {
        this.info = info;
        addDefaultLoaders(baseClassLoader);
    ...
    public ProviderFactory compareFactories(ProviderFactory p1, ProviderFactory p2) {
        ...
        if (DefaultIAMShieldSessionFactory.isInternal(p1) ^ DefaultIAMShieldSessionFactory.isInternal(p2)) {
            return DefaultIAMShieldSessionFactory.isInternal(p1) ? p2 : p1;
        }
    ...
    public synchronized IAMShieldDeploymentInfo getInfo() {
        return  info;
    }
