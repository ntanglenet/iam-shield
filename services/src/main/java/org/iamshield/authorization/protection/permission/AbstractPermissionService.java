...
public class AbstractPermissionService {
    ...
    public AbstractPermissionService(IAMShieldIdentity identity, ResourceServer resourceServer, AuthorizationProvider authorization) {
    ...
    private String createPermissionTicket(List<PermissionRequest> request) {
        ...
        String audience = Urls.realmIssuer(this.authorization.getIAMShieldSession().getContext().getUri().getBaseUri(), this.authorization.getRealm().getName());
        ...
        return this.authorization.getIAMShieldSession().tokens().encode(token);
    }
}