 ...
 */
public class IAMShieldIdentity implements Identity {
    ...
    public IAMShieldIdentity(IAMShieldSession keycloakSession) {
        this(Tokens.getAccessToken(keycloakSession), keycloakSession);
    }

    public IAMShieldIdentity(IAMShieldSession keycloakSession, IDToken token) {
        this(token, keycloakSession, keycloakSession.getContext().getRealm());
    }

    public IAMShieldIdentity(IDToken token, IAMShieldSession keycloakSession, RealmModel realm) {
        if (token == null) {
    ...
    public IAMShieldIdentity(AccessToken accessToken, IAMShieldSession keycloakSession) {
        this(accessToken, keycloakSession, keycloakSession.getContext().getRealm());
    }

    public IAMShieldIdentity(AccessToken accessToken, IAMShieldSession keycloakSession, RealmModel realm) {
        if (accessToken == null) {
            throw new ErrorResponseException("invalid_bearer_token", "Could not obtain bearer access_token from request.", Status.FORBIDDEN);
        }
        if (keycloakSession == null) {
