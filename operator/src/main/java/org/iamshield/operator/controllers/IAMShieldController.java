...
import org.iamshield.operator.update.UpdateLogicFactory;
...
public class IAMShieldController implements Reconciler<Keycloak> {
    ...
    public UpdateControl<Keycloak> reconcile(Keycloak kc, Context<Keycloak> context) {
        ...
        if (modifiedSpec) {
            ...
            Keycloak patchedKc = new IAMShieldBuilder()
                    .withNewMetadata()
        ...
        ContextUtils.storeDesiredStatefulSet(context, new IAMShieldDeploymentDependentResource().desired(kc, context));
        ...
        var statusAggregator = new IAMShieldStatusAggregator(kc.getStatus(), kc.getMetadata().getGeneration());
    ...
    public ErrorStatusUpdateControl<Keycloak> updateErrorStatus(Keycloak kc, Context<Keycloak> context, Exception e) {
        ...
        KeycloakStatus status = new IAMShieldStatusAggregator(kc.getStatus(), kc.getMetadata().getGeneration())
