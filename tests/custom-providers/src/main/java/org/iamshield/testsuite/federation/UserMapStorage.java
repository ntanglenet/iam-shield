...
import org.iamshield.models.credential.PasswordUserCredentialModel;
import org.iamshield.storage.*;
import org.iamshield.storage.adapter.AbstractUserAdapterFederatedStorage;
import org.iamshield.storage.federated.UserGroupMembershipFederatedStorage;
...
public class UserMapStorage implements UserLookupProvider, UserStorageProvider, UserRegistrationProvider, CredentialInputUpdater,
    ...
    public UserMapStorage(IAMShieldSession session, ComponentModel model, Map<String, String> userPasswords, ConcurrentMap<String, Set<String>> userGroups) {
    ...
    public boolean removeUser(RealmModel realm, UserModel user) {
        if (editMode == EditMode.READ_ONLY || editMode == EditMode.UNSYNCED) {
            log.warnf("User '%s' can't be deleted in LDAP as editMode is '%s'. Deleting user just from IAMShield DB, but he will be re-imported from LDAP again once searched in IAMShield", user.getUsername(), editMode.toString());
    ...
    public boolean removeUserByName(String userName) {
        if (editMode == EditMode.READ_ONLY || editMode == EditMode.UNSYNCED) {
            log.warnf("User '%s' can't be deleted in LDAP as editMode is '%s'. Deleting user just from IAMShield DB, but he will be re-imported from LDAP again once searched in IAMShield", userName, editMode.toString());
