...
public class IndexSchemaChangeTest {
    ...
    private static final FileDescriptor V1 = IAMShieldModelSchema.parseProtoSchema(TestModelV1.INSTANCE.getProtoFile());
    private static final FileDescriptor V2 = IAMShieldModelSchema.parseProtoSchema(TestModelV2.INSTANCE.getProtoFile());
    ...
    private static void doTest(String entity, boolean expectChanged) {
        var v1 = IAMShieldModelSchema.findEntity(V1, entity);
        var v2 = IAMShieldModelSchema.findEntity(V2, entity);
        Assert.assertTrue(v1.isPresent());
        Assert.assertTrue(v2.isPresent());
        Assert.assertEquals(expectChanged, IAMShieldIndexSchemaUtil.isIndexSchemaChanged(v1.get(), v2.get()));
    }
}
