...
package org.iamshield.models.sessions.infinispan.changes;
...
import org.iamshield.models.session.PersistentAuthenticatedClientSessionAdapter;
import org.iamshield.models.session.PersistentUserSessionAdapter;
...
import org.iamshield.models.sessions.infinispan.entities.SessionEntity;
...
public class JpaChangesPerformer<K, V extends SessionEntity> {
    ...
    private final TriConsumer<IAMShieldSession, Map.Entry<K, SessionUpdatesList<V>>, MergedUpdate<V>> processor;
     ...
     * {@link #write(IAMShieldSession)}. The invoker should register the change using the first method and applied them
     ...
     *
     * @param session The {@link IAMShieldSession} to access the database.
     */
    public void write(IAMShieldSession session) {
    ...
    private TriConsumer<IAMShieldSession, Map.Entry<K, SessionUpdatesList<V>>, MergedUpdate<V>> processor(String cacheName) {
    ...
    private static <K, V extends SessionEntity> void processClientSessionUpdate(IAMShieldSession session, Map.Entry<K, SessionUpdatesList<V>> entry, MergedUpdate<V> merged) {
    ...
    private static <V extends SessionEntity> void createClientSession(IAMShieldSession session, SessionEntityWrapper<V> sessionWrapper, UserSessionPersisterProvider userSessionPersister) {
        AuthenticatedClientSessionEntity entity = (AuthenticatedClientSessionEntity) sessionWrapper.getEntity();
    ...
    private static <K, V extends SessionEntity> void processUserSessionUpdate(IAMShieldSession session, Map.Entry<K, SessionUpdatesList<V>> entry, MergedUpdate<V> merged) {
        SessionUpdatesList<V> sessionUpdates = entry.getValue();
        SessionEntityWrapper<V> sessionWrapper = sessionUpdates.getEntityWrapper();
        RealmModel realm = sessionUpdates.getRealm();
        UserSessionPersisterProvider userSessionPersister = session.getProvider(UserSessionPersisterProvider.class);
    ...
    private static <K, V extends SessionEntity> void mergeUserSession(IAMShieldSession innerSession, Map.Entry<K, SessionUpdatesList<V>> entry, PersistentUserSessionAdapter userSessionModel, RealmModel realm, SessionUpdatesList<V> sessionUpdates, UserSessionPersisterProvider userSessionPersister, UserSessionEntity entity) {
