...
import org.iamshield.models.sessions.infinispan.changes.PersistentUpdate;
...
import org.iamshield.models.sessions.infinispan.changes.sessions.PersisterLastSessionRefreshStoreFactory;
...
public class InfinispanUserSessionProviderFactory implements UserSessionProviderFactory<UserSessionProvider>, ServerInfoAwareProviderFactory, EnvironmentDependentProviderFactory {
    ...
    @Override
    public UserSessionProvider create(IAMShieldSession session) {
            ...
            if (event instanceof PostMigrationEvent) {
                if (!useCaches) {
                    keyGenerator = new InfinispanKeyGenerator() {
                        @Override
                        protected <K> K generateKey(IAMShieldSession session, Cache<K, ?> cache, KeyGenerator<K> keyGenerator) {
                            return keyGenerator.getKey();
                        }
                    };
                } else {
                    IAMShieldModelUtils.runJobInTransaction(factory, (IAMShieldSession session) -> {
                ...
                UserSessionProvider provider1 = userRemovedEvent.getIAMShieldSession().getProvider(UserSessionProvider.class, getId());
    ...
    public void initializePersisterLastSessionRefreshStore(final IAMShieldSessionFactory sessionFactory) {
        IAMShieldModelUtils.runJobInTransaction(sessionFactory, session -> {
    ...
    private VolatileTransactions createVolatileTransaction(IAMShieldSession session) {
    ...
    private PersistentTransaction createPersistentTransaction(IAMShieldSession session) {
