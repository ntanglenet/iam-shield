...
import static org.iamshield.authorization.fgap.AdminPermissionsSchema.runWithoutAuthorization;
 ...
 * as cached data is used in instanced of {@link org.iamshield.models.cache.infinispan.entities.CachedRealm} which are shared
...
public class DefaultLazyLoader<S, D> implements LazyLoader<S, D> {
    ...
    @Override
    public D get(IAMShieldSession session, Supplier<S> sourceSupplier) {
        if (data == null) {
            synchronized (this) {
                if (data == null) {
