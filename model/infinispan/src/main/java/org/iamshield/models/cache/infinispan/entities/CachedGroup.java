...
public class CachedGroup extends AbstractRevisioned implements InRealm {
    ...
    public MultivaluedHashMap<String, String> getAttributes(IAMShieldSession session, Supplier<GroupModel> group) {
        return attributes.get(session, group);
    }

    public Set<String> getRoleMappings(IAMShieldSession session, Supplier<GroupModel> group) {
    ...
    public Set<String> getSubGroups(IAMShieldSession session, Supplier<GroupModel> group) {
