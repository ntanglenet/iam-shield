...
import static org.iamshield.jgroups.certificates.JGroupsCertificate.fromJson;
import static org.iamshield.jgroups.certificates.JGroupsCertificate.toJson;
...
public class DatabaseJGroupsCertificateProvider implements JGroupsCertificateProvider {
    ...
    private DatabaseJGroupsCertificateProvider(IAMShieldSessionFactory sessionFactory, Duration rotationPeriod) {
    ...
    public static DatabaseJGroupsCertificateProvider create(IAMShieldSessionFactory factory, Duration rotationPeriod) {
    ...
    private void init() {
        ...
        var cert = Retry.call(ignored -> IAMShieldModelUtils.runJobInTransactionWithResult(sessionFactory, this::loadOrCreateCertificate), STARTUP_RETRIES, STARTUP_RETRY_SLEEP_MILLIS);
        useCertificate(cert);
    ...
    public void rotateCertificate() {
        IAMShieldModelUtils.runJobInTransaction(sessionFactory, this::replaceCertificateFromDatabase);
    }
    ...
    private void doReload() {
        IAMShieldModelUtils.runJobInTransactionWithResult(sessionFactory, DatabaseJGroupsCertificateProvider::loadCertificateFromDatabase)
    ...
    private static Optional<String> loadCertificateFromDatabase(IAMShieldSession session) {
        return session.getProvider(ServerConfigStorageProvider.class).find(CERTIFICATE_ID);
    }

    private void replaceCertificateFromDatabase(IAMShieldSession session) {
    ...
    private JGroupsCertificate loadOrCreateCertificate(IAMShieldSession session) {
        var storage = session.getProvider(ServerConfigStorageProvider.class);
