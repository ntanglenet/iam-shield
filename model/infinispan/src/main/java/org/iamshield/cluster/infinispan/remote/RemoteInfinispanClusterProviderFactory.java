...
import org.iamshield.cluster.infinispan.InfinispanClusterProvider;
...
public class RemoteInfinispanClusterProviderFactory implements ClusterProviderFactory, RemoteInfinispanClusterProvider.SharedData, EnvironmentDependentProviderFactory {
    ...
    private synchronized void lazyInit(IAMShieldSession session) {
        if (workCache != null) {
            return;
        }
        var provider = session.getProvider(InfinispanConnectionProvider.class);
        executor = provider.getExecutor("cluster-provider");
        clusterStartupTime = initClusterStartupTime(provider.getRemoteCache(WORK_CACHE_NAME), (int) (session.getIAMShieldSessionFactory().getServerStartupTimestamp() / 1000));
