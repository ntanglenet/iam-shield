...
import static org.iamshield.config.CachingOptions.CACHE_EMBEDDED_PREFIX;
import static org.iamshield.connections.infinispan.InfinispanConnectionProvider.JBOSS_NODE_NAME;
import static org.iamshield.connections.infinispan.InfinispanConnectionProvider.JBOSS_SITE_NAME;
import static org.iamshield.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.MACHINE_NAME;
import static org.iamshield.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.NODE_NAME;
import static org.iamshield.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.PROVIDER_ID;
import static org.iamshield.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.RACK_NAME;
import static org.iamshield.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.SITE_NAME;
import static org.iamshield.spi.infinispan.impl.embedded.DefaultCacheEmbeddedConfigProviderFactory.TRACING;
...
import org.iamshield.connections.jpa.JpaConnectionProviderFactory;
import org.iamshield.connections.jpa.util.JpaUtils;
import org.iamshield.infinispan.util.InfinispanUtils;
import org.iamshield.jgroups.protocol.KEYCLOAK_JDBC_PING2;
import org.iamshield.jgroups.protocol.OPEN_TELEMETRY;
import org.iamshield.jgroups.header.TracerHeader;
...
public final class JGroupsConfigurator {
     ...
     * @param session The {@link IAMShieldSession} sessions for Database access.
     */
    public static void configureJGroups(Config.Scope config, ConfigurationBuilderHolder holder, IAMShieldSession session) {
        if (isLocal(holder)) {
            return;
        }
    ...
    private static void configureTls(ConfigurationBuilderHolder holder, IAMShieldSession session) {
    ...
    private static void configureDiscovery(ConfigurationBuilderHolder holder, IAMShieldSession session, boolean tracingEnabled) {
        ...
        var connectionFactory = (JpaConnectionProviderFactory) session.getIAMShieldSessionFactory().getProviderFactory(JpaConnectionProvider.class);

        String clusterName = transportOf(holder).attributes().attribute(CLUSTER_NAME).get();

        Address address = Retry.call(ignored -> IAMShieldModelUtils.runJobInTransactionWithResult(session.getIAMShieldSessionFactory(),
     ...
     */
    private static Address prepareJGroupsAddress(IAMShieldSession session, String clusterName) {
