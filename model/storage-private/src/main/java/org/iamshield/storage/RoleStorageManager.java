...
import org.iamshield.storage.role.RoleLookupProvider;
import org.iamshield.storage.role.RoleStorageProvider;
import org.iamshield.storage.role.RoleStorageProviderFactory;
import org.iamshield.storage.role.RoleStorageProviderModel;
import org.iamshield.utils.ServicesUtils;
...
public class RoleStorageManager implements RoleProvider {
    ...
    public RoleStorageManager(IAMShieldSession session, long roleStorageProviderTimeout) {
    ...
    public static RoleStorageProvider getStorageProvider(IAMShieldSession session, RealmModel realm, String componentId) {
        ComponentModel model = realm.getComponent(componentId);
        if (model == null) return null;
        RoleStorageProviderModel storageModel = new RoleStorageProviderModel(model);
        RoleStorageProviderFactory factory = (RoleStorageProviderFactory)session.getIAMShieldSessionFactory().getProviderFactory(RoleStorageProvider.class, model.getProviderId());
        if (factory == null) {
    ...
    public static <T> Stream<RoleStorageProviderModel> getStorageProviders(RealmModel realm, IAMShieldSession session, Class<T> type) {
    ...
    private static RoleStorageProviderFactory getRoleStorageProviderFactory(RoleStorageProviderModel model, IAMShieldSession session) {
        return (RoleStorageProviderFactory) session.getIAMShieldSessionFactory()
                .getProviderFactory(RoleStorageProvider.class, model.getProviderId());
    }

    public static RoleStorageProvider getStorageProviderInstance(IAMShieldSession session, RoleStorageProviderModel model, RoleStorageProviderFactory factory) {
     ...
     * the <code>org.iamshield.services.DefaultIAMShieldSessionFactory#getRoleStorageProviderTimeout()</code> property is used.
     * Default value is 3000 milliseconds and it's configurable.
     * See <code>org.iamshield.services.DefaultIAMShieldSessionFactory</code> for details.
     */
    @Override
     ...
     * Obtaining roles from an external role storage is time-bounded. In case the external role storage
     * isn't available at least roles from a local storage are returned. For this purpose
     * the <code>org.iamshield.services.DefaultIAMShieldSessionFactory#getRoleStorageProviderTimeout()} property is used.
     * Default value is 3000 milliseconds and it's configurable.
     * See <code>org.iamshield.services.DefaultIAMShieldSessionFactory</code> for details.
     */
    @Override
