...
import org.iamshield.storage.client.ClientStorageProviderFactory;
...
public class ClientStorageManager implements ClientProvider {
    ...
    public static ClientStorageProvider getStorageProvider(IAMShieldSession session, RealmModel realm, String componentId) {
        ComponentModel model = realm.getComponent(componentId);
        if (model == null) return null;
        ClientStorageProviderModel storageModel = new ClientStorageProviderModel(model);
        ClientStorageProviderFactory factory = (ClientStorageProviderFactory)session.getIAMShieldSessionFactory().getProviderFactory(ClientStorageProvider.class, model.getProviderId());
        if (factory == null) {
    ...
    private static <T> Stream<ClientStorageProviderModel> getStorageProviders(RealmModel realm, IAMShieldSession session, Class<T> type) {
    ...
    public static ClientStorageProvider getStorageProviderInstance(IAMShieldSession session, ClientStorageProviderModel model, ClientStorageProviderFactory factory) {
    ...
    private static ClientStorageProviderFactory getClientStorageProviderFactory(ClientStorageProviderModel model, IAMShieldSession session) {
        return (ClientStorageProviderFactory) session.getIAMShieldSessionFactory()
    ...
    public static boolean hasEnabledStorageProviders(IAMShieldSession session, RealmModel realm, Class<?> type) {
        return getStorageProviders(realm, session, type).anyMatch(ClientStorageProviderModel::isEnabled);
    }


    public ClientStorageManager(IAMShieldSession session, long clientStorageProviderTimeout) {
    ...
    protected Stream<ClientModel> query(PaginatedQuery paginatedQuery, RealmModel realm, Integer firstResult, Integer maxResults) {
        ...
        if (hasEnabledStorageProviders(session, realm, ClientLookupProvider.class)) {
            ...
            /*
              ...
              the {@link org.iamshield.services.DefaultIAMShieldSessionFactory#getClientStorageProviderTimeout()} property is used.
              Default value is 3000 milliseconds and it's configurable.
              See {@link org.iamshield.services.DefaultIAMShieldSessionFactory} for details.
             */
