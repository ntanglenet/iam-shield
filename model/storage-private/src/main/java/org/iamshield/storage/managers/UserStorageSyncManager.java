 ...
 */
package org.iamshield.storage.managers;
...
import org.iamshield.cluster.ExecutionResult;
...
import org.iamshield.timer.TimerProvider.TimerTaskContext;
...
public class UserStorageSyncManager {
     ...
     * @param timer
     */
    public static void bootstrapPeriodic(final IAMShieldSessionFactory sessionFactory, final TimerProvider timer) {
            ...
            public void run(IAMShieldSession session) {
                ...
                realms.forEach(realm -> {
                    ...
                    providers.forEachOrdered(provider -> {
                        UserStorageProviderFactory factory = (UserStorageProviderFactory) session.getIAMShieldSessionFactory().getProviderFactory(UserStorageProvider.class, provider.getProviderId());
    ...
    public static SynchronizationResult syncAllUsers(final IAMShieldSessionFactory sessionFactory, final String realmId, final UserStorageProviderModel provider) {
    ...
    public static SynchronizationResult syncChangedUsers(final IAMShieldSessionFactory sessionFactory, final String realmId, final UserStorageProviderModel provider) {
        UserStorageProviderFactory factory = (UserStorageProviderFactory) sessionFactory.getProviderFactory(UserStorageProvider.class, provider.getProviderId());
        if (!(factory instanceof ImportSynchronization) || !provider.isImportEnabled() || !provider.isEnabled()) {
            return SynchronizationResult.ignored();
    ...
    public static void notifyToRefreshPeriodicSyncAll(IAMShieldSession session, RealmModel realm, boolean removed) {
    ...
    public static void notifyToRefreshPeriodicSyncSingle(IAMShieldSession session, RealmModel realm, ComponentModel component, boolean removed) {
    ...
    public static void notifyToRefreshPeriodicSync(IAMShieldSession session, RealmModel realm, UserStorageProviderModel provider, boolean removed) {
        UserStorageProviderFactory factory = (UserStorageProviderFactory) session.getIAMShieldSessionFactory().getProviderFactory(UserStorageProvider.class, provider.getProviderId());
        if (!(factory instanceof ImportSynchronization) || !provider.isImportEnabled()) {
            return;
    ...
    protected static void refreshPeriodicSyncForProvider(final IAMShieldSessionFactory sessionFactory, TimerProvider timer, final UserStorageProviderModel provider, final RealmModel realm) {
    ...
    public static class UserStorageSyncTask implements Runnable {
        ...
        public UserStorageSyncTask(UserStorageProviderModel provider, RealmModel realm, IAMShieldSessionFactory sessionFactory, SyncMode syncMode) {
    ...
    private static void updateLastSyncInterval(final IAMShieldSessionFactory sessionFactory, UserStorageProviderModel provider, final String realmId, final int lastSync) {
        IAMShieldModelUtils.runJobInTransaction(sessionFactory, new IAMShieldSessionTask() {
    ...
    private static class UserStorageClusterListener implements ClusterListener {
        ...
        public UserStorageClusterListener(IAMShieldSessionFactory sessionFactory) {
            this.sessionFactory = sessionFactory;
        }
