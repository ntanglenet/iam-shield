...
public class MigrateTo24_0_0 extends RealmMigration {
    ...
    private void updateUserProfileSettings(IAMShieldSession session) {
    ...
    private void updateLdapProviderConfig(final IAMShieldSession session) {
        RealmModel realm = session.getContext().getRealm();
    ...
    private void createHS512ComponentModelKey(IAMShieldSession session) {
        RealmModel realm = session.getContext().getRealm();
        DefaultKeyProviders.createSecretProvider(realm);
    }

    private void bindFirstBrokerLoginFlow(IAMShieldSession session) {
        RealmModel realm = session.getContext().getRealm();
