...
package org.iamshield.connections.jpa;

import static org.iamshield.connections.jpa.util.JpaUtils.configureNamedQuery;
import static org.iamshield.connections.jpa.util.JpaUtils.getDatabaseType;
...
import org.iamshield.ServerStartupError;
import org.iamshield.common.util.StackUtil;
import org.iamshield.common.util.StringPropertyReplacer;
import org.iamshield.connections.jpa.support.EntityManagerProxy;
import org.iamshield.connections.jpa.updater.JpaUpdaterProvider;
...
public class DefaultJpaConnectionProviderFactory implements JpaConnectionProviderFactory, ServerInfoAwareProviderFactory {
    ...
    private IAMShieldSessionFactory factory;

    @Override
    public JpaConnectionProvider create(IAMShieldSession session) {
    ...
    private EntityManager createEntityManager(IAMShieldSession session, boolean sessionManaged) {
        EntityManager em;
        if (!jtaEnabled) {
            logger.trace("enlisting EntityManager in JpaIAMShieldTransaction");
        ...
        if (!jtaEnabled) {
            session.getTransactionManager().enlist(new JpaIAMShieldTransaction(em));
        }
        return em;
    }

    private void addSpecificNamedQueries(IAMShieldSession session, Connection connection) {
    ...
    protected void checkJtaEnabled(IAMShieldSessionFactory factory) {
    ...
    private void lazyInit(IAMShieldSession session) {
        if (emf == null) {
            synchronized (this) {
                if (emf == null) {
                    IAMShieldModelUtils.suspendJtaTransaction(session.getIAMShieldSessionFactory(), () -> {
                        logger.debugf("Initializing JPA connections%s", StackUtil.getShortStackTrace());

                        Map<String, Object> properties = new HashMap<>();

                        String unitName = "iamshield-default";
    ...
    protected void startGlobalStats(IAMShieldSession session, int globalStatsIntervalSecs) {
    ...
    void migration(MigrationStrategy strategy, boolean initializeEmpty, String schema, File databaseUpdateFile, Connection connection, IAMShieldSession session) {
    ...
    protected void update(Connection connection, String schema, IAMShieldSession session, JpaUpdaterProvider updater) {
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), new IAMShieldSessionTask() {
            @Override
            public void run(IAMShieldSession lockSession) {
    ...
    protected void export(Connection connection, String schema, File databaseUpdateFile, IAMShieldSession session, JpaUpdaterProvider updater) {
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), new IAMShieldSessionTask() {
            @Override
            public void run(IAMShieldSession lockSession) {
                DBLockManager dbLockManager = new DBLockManager(lockSession);
    ...
    private void migrateModel(IAMShieldSession session) {
        ...
        try {
            IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), MigrationModelManager::migrate);
        } finally {
            dbLock.releaseLock();
        }
    }
