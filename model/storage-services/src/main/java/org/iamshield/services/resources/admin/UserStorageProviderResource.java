...
import org.iamshield.services.managers.LDAPServerCapabilitiesManager;
...
public class UserStorageProviderResource {
    ...
    public UserStorageProviderResource(IAMShieldSession session, AdminPermissionEvaluator auth, AdminEventBuilder adminEvent) {
    ...
    public static String getErrorCode(Throwable throwable) {
        if (throwable instanceof org.iamshield.models.ModelException) {
    ...
    public SynchronizationResult syncUsers(@PathParam("id") String id,
        ...
        if ("triggerFullSync".equals(action)) {
            try {
                syncResult = syncManager.syncAllUsers(session.getIAMShieldSessionFactory(), realm.getId(), providerModel);
            } catch(Exception e) {
            ...
            try {
                syncResult = syncManager.syncChangedUsers(session.getIAMShieldSessionFactory(), realm.getId(), providerModel);
            } catch(Exception e) {
                String errorMsg = getErrorCode(e);
                throw ErrorResponse.error(errorMsg, Response.Status.BAD_REQUEST);
            }
