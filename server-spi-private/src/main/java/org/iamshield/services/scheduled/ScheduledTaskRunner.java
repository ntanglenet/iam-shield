...
package org.iamshield.services.scheduled;

import org.jboss.logging.Logger;
import org.iamshield.logging.MappedDiagnosticContextUtil;
import org.iamshield.models.IAMShieldSession;
import org.iamshield.models.IAMShieldSessionFactory;
import org.iamshield.models.utils.IAMShieldModelUtils;
import org.iamshield.timer.ScheduledTask;
import org.iamshield.timer.TaskRunner;
...
public class ScheduledTaskRunner implements TaskRunner {
    ...
    protected final IAMShieldSessionFactory sessionFactory;

    protected final ScheduledTask task;

    protected final int transactionLimit;

    public ScheduledTaskRunner(IAMShieldSessionFactory sessionFactory, ScheduledTask task) {
        this(sessionFactory, task, 0);
    }

    public ScheduledTaskRunner(IAMShieldSessionFactory sessionFactory, ScheduledTask task, int transactionLimit) {
    ...
    public void run() {
        ...
        try {
            tracing.trace("ScheduledTaskRunner", task.getTaskName() + ".run", span -> {
                IAMShieldModelUtils.runJobInTransaction(sessionFactory, new NamedSessionTask("Scheduled task: " + task.getTaskName()) {

                    @Override
                    public void run(IAMShieldSession session) {
                        try {
                            if (transactionLimit != 0) {
                                IAMShieldModelUtils.setTransactionLimit(sessionFactory, transactionLimit);
                            }

                            runTask(session);
                        } finally {
                            if (transactionLimit != 0) {
                                IAMShieldModelUtils.setTransactionLimit(sessionFactory, 0);
                            }
                        }
                    }
                });
    ...
    protected void runTask(IAMShieldSession session) {
        task.run(session);
