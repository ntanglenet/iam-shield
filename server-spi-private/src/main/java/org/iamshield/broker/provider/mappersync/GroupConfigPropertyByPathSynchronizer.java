...
import static org.iamshield.models.utils.IAMShieldModelUtils.GROUP_PATH_SEPARATOR;
...
public class GroupConfigPropertyByPathSynchronizer implements ConfigSynchronizer<GroupModel.GroupPathChangeEvent> {
    ...
    public void handleEvent(GroupModel.GroupPathChangeEvent event) {
        ...
        event.getIAMShieldSession().identityProviders().getMappersStream(Map.of(ConfigConstants.GROUP, event.getPreviousPath()), null, null)
                .forEach(idpMapper -> {
        ...
        event.getIAMShieldSession().identityProviders().getMappersStream(
