...
public class EventBuilder {
    ...
    public EventBuilder(RealmModel realm, IAMShieldSession session, ClientConnection clientConnection) {
        this(realm, session);
        ipAddress(clientConnection.getRemoteHost());
    }

    public EventBuilder(RealmModel realm, IAMShieldSession session) {
    ...
    private static EventStoreProvider getEventStoreProvider(IAMShieldSession session) {
    ...
    private static List<EventListenerProvider> getEventListeners(IAMShieldSession session, RealmModel realm) {
        HashSet<String> realmListeners = new HashSet<>(realm.getEventsListenersStream().toList());
        List<EventListenerProvider> result = session.getIAMShieldSessionFactory().getProviderFactoriesStream(EventListenerProvider.class)
    ...
    private EventBuilder(IAMShieldSession session, EventStoreProvider store, List<EventListenerProvider> listeners, RealmModel realm, Event event) {
    ...
    private void send(boolean sendImmediately) {
        ...
        if (sendImmediately) {
            IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), session.getContext(), innerSession -> {
