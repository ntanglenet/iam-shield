...
import org.iamshield.models.dblock.DBLockProviderFactory;
...
public class DBLockTest extends IAMShieldModelTest {
    ...
    public void simpleNestedLockTest() {
        inComittedTransaction(1, (session , i) -> {
            ...
            try {
                ...
                IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionLC2) -> {
    ...
    private void testLockConcurrentlyInternal(IAMShieldSession sessionLC, DBLockProvider.Namespace lock) {
        long startupTime = System.currentTimeMillis();

        final Semaphore semaphore = new Semaphore();
        final IAMShieldSessionFactory sessionFactory = sessionLC.getIAMShieldSessionFactory();
    ...
    private void testTwoLocksCurrentlyInternal(IAMShieldSession sessionLC, DBLockProvider.Namespace lock1, DBLockProvider.Namespace lock2) {
        final Semaphore semaphore = new Semaphore();
        final IAMShieldSessionFactory sessionFactory = sessionLC.getIAMShieldSessionFactory();
        List<Thread> threads = new LinkedList<>();
        ...
        for (int i = 0; i < 2; i++) {
            ...
            Thread thread = new Thread(() -> {
                IntStream.range(0, ITERATIONS_PER_THREAD_LONG).parallel().forEach(j -> {
                    try {
                        IAMShieldModelUtils.runJobInTransaction(sessionFactory, session1 -> lock(session1, lock, semaphore, SLEEP_TIME_MILLIS_FOR_TWO_LOCKS));
                    } catch (RuntimeException e) {
                        semaphore.setException(e);
                    }
                });
    ...
    private void testTwoNestedLocksCurrentlyInternal(IAMShieldSession sessionLC, DBLockProvider.Namespace lockTop, DBLockProvider.Namespace lockInner) {
        final Semaphore semaphore = new Semaphore();
        final IAMShieldSessionFactory sessionFactory = sessionLC.getIAMShieldSessionFactory();
        List<Thread> threads = new LinkedList<>();
        // launch two threads and expect an error because the locks are different
                ...
                for (int j = 0; j < ITERATIONS_PER_THREAD_MEDIUM; j++) {
                    try {
                        if (nested) {
                            ...
                            IAMShieldModelUtils.runJobInTransaction(sessionFactory,
    ...
    private void lock(IAMShieldSession session, DBLockProvider.Namespace lock, Semaphore semaphore) {
        this.lock(session, lock, semaphore, SLEEP_TIME_MILLIS);
    }

    private void lock(IAMShieldSession session, DBLockProvider.Namespace lock, Semaphore semaphore, long sleepTime) {
        DBLockProvider dbLock = new DBLockManager(session).getDBLock();
    ...
    private void nestedTwoLevelLock(IAMShieldSession session, DBLockProvider.Namespace lockTop,
        ...
        try {
            ...
            IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(),
