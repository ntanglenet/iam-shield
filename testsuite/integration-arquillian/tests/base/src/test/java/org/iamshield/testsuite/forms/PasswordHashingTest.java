...
import org.iamshield.credential.hash.PasswordHashProviderFactory;
import org.iamshield.credential.hash.Pbkdf2PasswordHashProvider;
import org.iamshield.credential.hash.Pbkdf2PassHashProviFactory;
...
import org.iamshield.testsuite.util.DefaultPasswordHash;
 ...
 */
public class PasswordHashingTest extends AbstractTestRealmIAMShieldTest {
    ...
    public void testPasswordRehashedOnAlgorithmChanged() throws Exception {
        setPasswordPolicy("hashAlgorithm(" + Pbkdf2Sha256PassHashProviFactory.ID + ") and hashIterations(1)");
        ...
        assertEquals(Pbkdf2Sha256PassHashProviFactory.ID, credential.getPasswordCredentialData().getAlgorithm());

        assertEncoded(credential, password, credential.getPasswordSecretData().getSalt(), "PBKDF2WithHmacSHA256", 1);

        setPasswordPolicy("hashAlgorithm(" + Pbkdf2PassHashProviFactory.ID + ") and hashIterations(1)");
        ...
        assertEquals(Pbkdf2PassHashProviFactory.ID, credential.getPasswordCredentialData().getAlgorithm());
    ...
    public void testPasswordRehashedToDefaultProviderIfHashAlgorithmRemoved() {
        setPasswordPolicy("hashAlgorithm(" + Pbkdf2Sha256PassHashProviFactory.ID + ")");
    ...
    public void testPasswordNotRehasedUnchangedIterations() {
        ...
        setPasswordPolicy("hashIterations(" + Pbkdf2Sha512PassHashProviFactory.DEFAULT_ITERATIONS + ")");
    ...
    public void testPasswordRehashedWhenCredentialImportedWithDifferentKeySize() {
        setPasswordPolicy("hashAlgorithm(" + Pbkdf2Sha512PassHashProviFactory.ID + ") and hashIterations(" + Pbkdf2Sha512PassHashProviFactory.DEFAULT_ITERATIONS + ")");
        ...
        Pbkdf2PasswordHashProvider specificKeySizeHashProvider = new Pbkdf2PasswordHashProvider(Pbkdf2Sha512PassHashProviFactory.ID,
                Pbkdf2Sha512PassHashProviFactory.PBKDF2_ALGORITHM,
                Pbkdf2Sha512PassHashProviFactory.DEFAULT_ITERATIONS,
                0,
                256);
    ...
    public void testPbkdf2Sha1() throws Exception {
        setPasswordPolicy("hashAlgorithm(" + Pbkdf2PassHashProviFactory.ID + ")");
        String username = "testPbkdf2Sha1";
        final String password = createUser(username);

        PasswordCredentialModel credential = PasswordCredentialModel.createFromCredentialModel(fetchCredentials(username));
        assertEncoded(credential, password, credential.getPasswordSecretData().getSalt(), "PBKDF2WithHmacSHA1", Pbkdf2PassHashProviFactory.DEFAULT_ITERATIONS);
    }
    ...
    public void testDefault() throws Exception {
        ...
        if (notFips()) {
            ...
            assertEncoded(credential, password, credential.getPasswordSecretData().getSalt(), "PBKDF2WithHmacSHA512", Pbkdf2Sha512PassHashProviFactory.DEFAULT_ITERATIONS);
        }
    }
    ...
    public void testPbkdf2Sha256() throws Exception {
        ...
        PasswordCredentialModel credential = PasswordCredentialModel.createFromCredentialModel(fetchCredentials(username));
        assertEncoded(credential, password, credential.getPasswordSecretData().getSalt(), "PBKDF2WithHmacSHA256", Pbkdf2Sha256PassHashProviFactory.DEFAULT_ITERATIONS);
    }

    @Test
    public void testPbkdf2Sha512() throws Exception {
        setPasswordPolicy("hashAlgorithm(" + Pbkdf2Sha512PassHashProviFactory.ID + ")");
    ...
    public void testPbkdf2Sha256WithPadding() throws Exception {
        ...
        try {
            ...
            assertEncoded(credential, password, credential.getPasswordSecretData().getSalt(), "PBKDF2WithHmacSHA256", Pbkdf2Sha256PassHashProviFactory.DEFAULT_ITERATIONS, false);
    ...
    private void assertEncoded(PasswordCredentialModel credential, String password, byte[] salt, String algorithm, int iterations, boolean expectedSuccess) throws Exception {
        if (algorithm.startsWith("PBKDF2")) {
            ...
            if (Pbkdf2Sha256PassHashProviFactory.ID.equals(credential.getPasswordCredentialData().getAlgorithm())) {
                keyLength = 256;
            }
    ...
    private int configurePaddingForKeycloak(int paddingLength) {
        return testingClient.server("test").fetch(session -> {
            Pbkdf2Sha256PassHashProviFactory factory = (Pbkdf2Sha256PassHashProviFactory) session.getIAMShieldSessionFactory().getProviderFactory(PasswordHashProvider.class, Pbkdf2Sha256PassHashProviFactory.ID);
...
//    @Test
    public void testBenchmarkPasswordHashingConfigurations() {
        ...
        var comparisons = List.of(
                ...
                new PasswordHashComparison(new Pbkdf2Sha256PassHashProviFactory(), 27_500, 600_000),
                new PasswordHashComparison(new Pbkdf2Sha512PassHashProviFactory(), 30_000, 210_000)
        );
