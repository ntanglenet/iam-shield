 ...
 */
public class UserSessionProviderOfflineTest extends AbstractTestRealmIAMShieldTest {
    ...
    public void testOfflineSessionsCrud(IAMShieldSession session) {
        Map<String, Set<String>> offlineSessions = new HashMap<>();

        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), session.getContext(), currentSession -> {
    ...
    public void testOnRealmRemoved(IAMShieldSession session) {
        ...
        String realmId = IAMShieldModelUtils.runJobInTransactionWithResult(session.getIAMShieldSessionFactory(), currentSession -> {
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), currentSession -> {
            UserSessionManager sessionManager = new UserSessionManager(currentSession);
    ...
    @Test
    @ModelTest
    public void testOnUserRemoved(IAMShieldSession session) {
        AtomicReference<String> userSessionID = new AtomicReference<>();
        String realmId = IAMShieldModelUtils.runJobInTransactionWithResult(session.getIAMShieldSessionFactory(), currentSession -> {
            RealmModel fooRealm = currentSession.realms().createRealm("foo");
            currentSession.getContext().setRealm(fooRealm);
    ...
    private static Set<String> createOfflineSessionIncludeClientSessions(IAMShieldSession session, UserSessionModel
            userSession) {
    ...
    private static AuthenticatedClientSessionModel createClientSession(IAMShieldSession sessionParam, ClientModel
    ...
    private static UserSessionModel[] createSessions(IAMShieldSession currentSession) {
    ...
    public static void reloadState(IAMShieldSession session) {
        reloadState(session, false);
    }

    public static void reloadState(IAMShieldSession currentSession, Boolean initialConfig) {
        RealmModel realm = currentSession.realms().getRealmByName("test");
        if (initialConfig) {
