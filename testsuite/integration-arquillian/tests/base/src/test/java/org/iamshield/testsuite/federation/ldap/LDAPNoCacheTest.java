 ...
 * and changes are supposed to be immediately visible on IAMShield side
 *
...
public class LDAPNoCacheTest extends AbstractLDAPTest {
    ...
    protected void afterImportTestRealm() {
        testingClient.server().run(session -> {
            ...
            // Switch mappers to "Always read value from LDAP". Changed attributes in LDAP should be immediately visible on IAMShield side
    ...
    private static void checkEmailAddressMultipleVariants(IAMShieldTestingClient testingClient, String username, String newEmail) {
        testingClient.server().run((IAMShieldSession session) -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
    ...
    public void resetPasswordLink() throws IOException, MessagingException {
        ...
        // Send and email to the current email address of john. This will sync john to the IAMShield DB
    ...
    private static void changeEmailAddressInLDAP(IAMShieldTestingClient testingClient, String newEmail) {
        testingClient.server().run((IAMShieldSession session) -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
