...
public class LDAPSpecialCharsTest extends AbstractLDAPTest {
    ...
    public void test03_specialCharUserJoiningSpecialCharGroup() {
        testingClient.server().run(session -> {
            ...
            GroupModel specialGroup = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group-spec,ia*l_characžter)s");
            Assert.assertNotNull(specialGroup);

            specialUser.joinGroup(specialGroup);

            GroupModel groupWithSlashes = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group/with/three/slashes");
