...
//    }
    ...
    public void test02duplicateUsernameAndEmailSync() {
        ...
        testingClient.server().run(session -> {
            ...
            SynchronizationResult result = new UserStorageSyncManager().syncAllUsers(session.getIAMShieldSessionFactory(), ctx.getRealm().getId(), ctx.getLdapModel());
        ...
        testingClient.server().run(session -> {
            ...
            result = new UserStorageSyncManager().syncAllUsers(session.getIAMShieldSessionFactory(), ctx.getRealm().getId(), ctx.getLdapModel());
    ...
    public void test08LDAPGroupSyncAfterGroupRename() {
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel kcGroup1 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group1");
            String descriptionAttrName = LDAPTestUtils.getGroupDescriptionLDAPAttrName(ctx.getLdapProvider());
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel kcGroup5 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group5");
            String descriptionAttrName = LDAPTestUtils.getGroupDescriptionLDAPAttrName(ctx.getLdapProvider());
    ...
    public void test09MembershipUsingDifferentAttributes() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel user8Group = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/user8group");
