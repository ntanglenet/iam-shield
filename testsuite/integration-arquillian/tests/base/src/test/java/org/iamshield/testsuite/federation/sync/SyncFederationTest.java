...
package org.iamshield.testsuite.federation.sync;
...
public class SyncFederationTest extends AbstractAuthTest {
    ...
    public void test01PeriodicSyncOnCreate() {
        ...
        testingClient.server().run(session -> {
            ...
            while (true) {
                SynchronizationResult result = UserStorageSyncManager.syncChangedUsers(session.getIAMShieldSessionFactory(), appRealm.getId(), dummyModel);
                if (result.isIgnored()) {
    ...
    public void test02PeriodicSyncOnUpdate() {
        ...
        testingClient.server().run(session -> {
            ...
            // This sync is here just to ensure that we have lock (doublecheck that periodic sync, which was possibly triggered before canceling timer is finished too)
            while (true) {
                SynchronizationResult result = usersSyncManager.syncChangedUsers(session.getIAMShieldSessionFactory(), appRealm.getId(), dummyModel);
                if (result.isIgnored()) {
                    log.infof("Still waiting for lock before periodic sync is finished", result.toString());
                    sleep(1000);
                } else {
