...
import org.iamshield.storage.ldap.mappers.membership.MembershipType;
...
import static org.iamshield.testsuite.util.LDAPTestUtils.getGroupDescriptionLDAPAttrName;
...
public class LDAPGroupMapperTest extends AbstractLDAPTest {
    ...
    protected void test01_ldapOnlyGroupMappings(boolean importEnabled) {
        testingClient.server().run(session -> {
            ...
            GroupModel group1 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group1");
            john.joinGroup(group1);

            // This group should already exists as it was imported from LDAP
            GroupModel group11 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group1/group11");
            mary.joinGroup(group11);

            // This group should already exists as it was imported from LDAP
            GroupModel group12 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group1/group12");
            john.joinGroup(group12);
            mary.joinGroup(group12);

            // This group should already exists as it was imported from LDAP
            GroupModel groupWithSlashesInName = IAMShieldModelUtils.findGroupByPath(session, appRealm, "Team 2016/2017");
            john.joinGroup(groupWithSlashesInName);
            mary.joinGroup(groupWithSlashesInName);

            // This group should already exists as it was imported from LDAP
            GroupModel groupChildWithSlashesInName = IAMShieldModelUtils.findGroupByPath(session, appRealm, "defaultGroup1/Team Child 2018/2019");
            john.joinGroup(groupChildWithSlashesInName);
            mary.joinGroup(groupChildWithSlashesInName);

            Assert.assertEquals("Team SubChild 2020/2021", IAMShieldModelUtils.findGroupByPath(session, appRealm, "defaultGroup1/Team Child 2018/2019/Team SubChild 2020/2021").getName());
            Assert.assertEquals("defaultGroup14", IAMShieldModelUtils.findGroupByPath(session, appRealm, "defaultGroup13/Team SubChild 2022/2023/A/B/C/D/E/defaultGroup14").getName());
            Assert.assertEquals("Team SubChild 2026/2027", IAMShieldModelUtils.findGroupByPath(session, appRealm, "Team Root 2024/2025/A/B/C/D/defaultGroup15/Team SubChild 2026/2027").getName());
        });
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel group11 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group1/group11");
            GroupModel group12 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group1/group12");
            GroupModel groupTeam20162017 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "Team 2016/2017");
            GroupModel groupTeamChild20182019 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "defaultGroup1/Team Child 2018/2019");
            UserModel john = session.users().getUserByUsername(appRealm, "johnkeycloak");
            UserModel mary = session.users().getUserByUsername(appRealm, "marykeycloak");
    ...
    public void test04_groupReferencingNonExistentMember() {
        testingClient.server().run(session -> {
            ...
            GroupModel kcGroup2 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group2");
    ...
    public void test06_addingUserToNewKeycloakGroup() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel group4 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group4");
            john.joinGroup(group4);

            GroupModel group31 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group3/group31");
            GroupModel group32 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group3/group32");

            john.joinGroup(group31);
            john.joinGroup(group32);

            GroupModel group14 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group1/group14");
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel group14 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group1/group14");
            GroupModel group3 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group3");
            GroupModel group31 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group3/group31");
            GroupModel group32 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group3/group32");
            GroupModel group4 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/group4");
    ...
    public void test07_newUserDefaultGroupsImportModeTest() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel defaultGroup11 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/defaultGroup1/defaultGroup11");
            Assert.assertNotNull(defaultGroup11);

            GroupModel defaultGroup12 = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/defaultGroup1/defaultGroup12");
    ...
    public void test08_ldapOnlyGroupMappingsRanged() {
        testingClient.server().run(session -> {
            ...
            GroupModel kcBigGroup = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/biggroup");
    ...
    public void test09_emptyMemberOnDeletionWorks() {
        testingClient.server().run(session -> {
            ...
            GroupModel kcDeleteGroup = IAMShieldModelUtils.findGroupByPath(session, appRealm, "/deletegroup");
            UserModel mary = session.users().getUserByUsername(appRealm, "marykeycloak");
    ...
    public void test10_multipleGroupMappersInDifferentPaths() {
        ...
        testingClient.server(TEST_REALM_NAME).run(session -> {
            ...
            GroupModel groupA = IAMShieldModelUtils.findGroupByPath(session, realm, "/ldap-groups-1/GroupA");
            GroupModel groupB = IAMShieldModelUtils.findGroupByPath(session, realm, "/ldap-groups-1/GroupB");
            GroupModel groupC = IAMShieldModelUtils.findGroupByPath(session, realm, "/ldap-groups-2/GroupC");
