...
import static org.iamshield.userprofile.config.UPConfigUtils.ROLE_ADMIN;
import static org.iamshield.userprofile.config.UPConfigUtils.ROLE_USER;
import static org.iamshield.userprofile.config.UPConfigUtils.parseSystemDefaultConfig;
...
import org.iamshield.representations.idm.AbstractUserRepresentation;
...
import org.iamshield.representations.userprofile.config.UPGroup;
...
import org.iamshield.userprofile.AttributeGroupMetadata;
...
import org.iamshield.representations.userprofile.config.UPAttributeSelector;
...
import org.iamshield.userprofile.Attributes;
import org.iamshield.userprofile.UserProfile;
import org.iamshield.userprofile.UserProfileConstants;
import org.iamshield.userprofile.UserProfileContext;
import org.iamshield.userprofile.UserProfileProvider;
import org.iamshield.userprofile.UserProfileUtil;
import org.iamshield.userprofile.ValidationException;
import org.iamshield.userprofile.config.UPConfigUtils;
import org.iamshield.userprofile.validator.MultiValueValidator;
import org.iamshield.userprofile.validator.PersonNameProhibitedCharactersValidator;
import org.iamshield.userprofile.validator.UsernameIDNHomographValidator;
...
public class UserProfileTest extends AbstractUserProfileTest {
    ...
    public void configureTestRealm(RealmRepresentation testRealm) {
        ...
        ClientRepresentation client = IAMShieldModelUtils.createClient(testRealm, "client-a");
        ...
        IAMShieldModelUtils.createClient(testRealm, "client-b");
    }
    ...
    public void testReadOnlyAllowed() throws Exception {
        ...
        userRep.setEmail(org.iamshield.models.utils.IAMShieldModelUtils.generateId() + "@keycloak.org");
    ...
    private static void testIdempotentProfile(IAMShieldSession session) {
    ...
    private static void testCustomAttributeInAnyContext(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();

        attributes.put(UserModel.USERNAME, org.iamshield.models.utils.IAMShieldModelUtils.generateId());
        attributes.put(UserModel.FIRST_NAME, "John");
        attributes.put(UserModel.LAST_NAME, "Doe");
        attributes.put(UserModel.EMAIL, org.iamshield.models.utils.IAMShieldModelUtils.generateId() + "@keycloak.org");
    ...
    private static void testEmptyAttributeRemoved(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testResolveProfile(IAMShieldSession session) {
        configureAuthenticationSession(session);

        Map<String, Object> attributes = new HashMap<>();

        attributes.put(UserModel.USERNAME, org.iamshield.models.utils.IAMShieldModelUtils.generateId() + "@keycloak.org");
        attributes.put(UserModel.FIRST_NAME, "John");
        attributes.put(UserModel.LAST_NAME, "Doe");
        attributes.put(UserModel.EMAIL, org.iamshield.models.utils.IAMShieldModelUtils.generateId() + "@keycloak.org");
    ...
    public void testValidation() throws IOException {
        ...
        getTestingClient().server(TEST_REALM_NAME).run((IAMShieldSession session) -> testNonAsciiEmailValidator(session, false));
        try (RealmAttributeUpdater updater = new RealmAttributeUpdater(testRealm())
            ...
            getTestingClient().server(TEST_REALM_NAME).run((IAMShieldSession session) -> testNonAsciiEmailValidator(session, true));
        }
    }

    private static void failValidationWhenEmptyAttributes(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
        UserProfileProvider provider = session.getProvider(UserProfileProvider.class);
    ...
    private static void testAttributeValidation(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
        UserProfileProvider provider = session.getProvider(UserProfileProvider.class);
    ...
    private static void testEmailAsUsernameValidation(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
        UserProfileProvider provider = session.getProvider(UserProfileProvider.class);
        provider.setConfiguration(null);
        UserProfile profile;
    ...
    private static void testNonAsciiEmailValidator(IAMShieldSession session, boolean success) {
        Map<String, Object> attributes = new HashMap<>();
        UserProfileProvider provider = session.getProvider(UserProfileProvider.class);
        List<ValidationError> errors = new ArrayList<>();
    ...
    private static void testValidateComplianceWithUserProfile(IAMShieldSession session) {
        RealmModel realm = session.getContext().getRealm();
    ...
    private static void testGetProfileAttributes(IAMShieldSession session) {
        RealmModel realm = session.getContext().getRealm();
        UserModel user = session.users().addUser(realm, org.iamshield.models.utils.IAMShieldModelUtils.generateId());
        user.setFirstName("John");
        user.setLastName("John");
        user.setEmail(org.iamshield.models.utils.IAMShieldModelUtils.generateId() + "@keycloak.org");
    ...
    private static void testGetProfileAttributeGroups(IAMShieldSession session) {
        RealmModel realm = session.getContext().getRealm();
        UserModel user = session.users().addUser(realm, org.iamshield.models.utils.IAMShieldModelUtils.generateId());
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testCreateAndUpdateUser(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        ...
        Map<String, Object> attributes = new HashMap<>();
        String userName = org.iamshield.models.utils.IAMShieldModelUtils.generateId();
    ...
    private static void testReadonlyUpdates(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testReadonlyEmailCannotBeUpdated(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testUpdateEmail(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testDoNotUpdateUndefinedAttributes(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testComponentModelId(IAMShieldSession session) {
    ...
    private static void testInvalidConfiguration(IAMShieldSession session) {
        try {
    ...
    private static void testResetConfiguration(IAMShieldSession session) {
    ...
    private static void testDefaultConfig(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
    ...
    private static void testCustomValidationForUsername(IAMShieldSession session) {
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testRemoveDefaultValidationFromUsername(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
    ...
    private static void testHomographValidator(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testOptionalAttributes(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    public void testPersonNameProhibitedCharsValidator(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    @Test
    @ModelTest(realmName = "test")
    public void testUriValidator(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testCustomAttributeRequired(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
        UPAttribute attribute = new UPAttribute();
    ...
    private static void testNullAttributesInConfig(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testCustomAttributeOptional(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
        UPAttribute attribute = new UPAttribute();
    ...
    private static void testRequiredIfUser(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testRequiredIfAdmin(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    public void testEmailRequired(IAMShieldSession session) {
        RealmModel realm = session.getContext().getRealm();
    ...
    private static void testNoValidationsIfUserReadOnly(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testNoValidationsIfAdminReadOnly(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testIgnoreReadOnlyAttribute(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testReadOnlyInternalAttributeValidation(IAMShieldSession session) {
        RealmModel realm = session.getContext().getRealm();
    ...
    @Test
    @ModelTest
    public void testRequiredByClientScope(IAMShieldSession session) {
    ...
    @Test
    @ModelTest
    public void testRequiredByOptionalClientScope(IAMShieldSession session) {
        RealmModel realm = session.realms().getRealmByName("test");
        session.getContext().setRealm(realm);
    ...
    private static void testConfigurationInvalidScope(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testUsernameAndEmailPermissionNotSetIfEmpty(IAMShieldSession session){
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig config = provider.getConfiguration();
    ...
    private static void testDoNotRemoveAttributes(IAMShieldSession session) {
        Map<String, Object> attributes = new HashMap<>();
    ...
    private static void testRemoveEmptyRootAttribute(IAMShieldSession session) {
        Map<String, List<String>> attributes = new HashMap<>();

        attributes.put(UserModel.USERNAME, List.of(org.iamshield.models.utils.IAMShieldModelUtils.generateId()));
    ...
    private static void testRemoveOptionalAttributesFromDefaultConfigIfNotSet(IAMShieldSession session) {
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testUnmanagedPolicy(IAMShieldSession session) {
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testOptionalRootAttributesAsUnmanagedAttribute(IAMShieldSession session) {
        UPConfig config = UPConfigUtils.parseSystemDefaultConfig();
        UserProfileProvider provider = getUserProfileProvider(session);
        provider.setConfiguration(config);
        Map<String, String> rawAttributes = new HashMap<>();
        rawAttributes.put(UserModel.USERNAME, org.iamshield.models.utils.IAMShieldModelUtils.generateId() + "@keycloak.org");
        rawAttributes.put(UserModel.EMAIL, org.iamshield.models.utils.IAMShieldModelUtils.generateId() + "@keycloak.org");
    ...
    private static void testAttributeNormalization(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
    ...
    private static void testEmailAttributeInUpdateEmailContext(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        String userName = org.iamshield.models.utils.IAMShieldModelUtils.generateId();
        Map<String, String> attributes = new HashMap<>();
    ...
    private static void testEmailAnnotationsInAccountContext(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        String userName = org.iamshield.models.utils.IAMShieldModelUtils.generateId();
        Map<String, String> attributes = new HashMap<>();
    ...
    private static void testInvalidConfigDefaultValue(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
    ...
    private static void testDefaultValue(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig upConfig = UPConfigUtils.parseSystemDefaultConfig();
        UPAttribute foo = new UPAttribute("foo", new UPAttributePermissions(Set.of(), Set.of(UserProfileConstants.ROLE_ADMIN)));
        foo.setDefaultValue("def");
    ...
    private static void testNoDefaultValueForRootAttributes(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig upConfig = UPConfigUtils.parseSystemDefaultConfig();
    ...
    private static void testMultivalued(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
        UPConfig upConfig = UPConfigUtils.parseSystemDefaultConfig();
        provider.setConfiguration(upConfig);
        String userName = org.iamshield.models.utils.IAMShieldModelUtils.generateId();
    ...
    private static void testDefaultConfigWhenComponentConfigIsNotSet(IAMShieldSession session) {
        UserProfileProvider provider = getUserProfileProvider(session);
