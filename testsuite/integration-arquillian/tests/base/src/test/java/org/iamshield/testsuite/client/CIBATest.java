...
import static org.iamshield.protocol.oidc.grants.ciba.channel.AuthenticationChannelResponse.Status.CANCELLED;
import static org.iamshield.protocol.oidc.grants.ciba.channel.AuthenticationChannelResponse.Status.SUCCEED;
import static org.iamshield.protocol.oidc.grants.ciba.channel.AuthenticationChannelResponse.Status.UNAUTHORIZED;
import static org.iamshield.testsuite.Assert.assertExpiration;
import static org.iamshield.testsuite.admin.AbstractAdminTest.loadJson;
import static org.iamshield.testsuite.util.ClientPoliciesUtil.createAnyClientConditionConfig;
...
import static org.iamshield.testsuite.util.ClientPoliciesUtil.createSecureCibaAuthenticationRequestSigningAlgorithmExecutorConfig;
import static org.iamshield.testsuite.util.ClientPoliciesUtil.createTestRaiseExeptionExecutorConfig;
...
import org.iamshield.models.CibaConfig;
import org.iamshield.protocol.oidc.OIDCAdvancedConfigWrapper;
import org.iamshield.protocol.oidc.grants.ciba.CibaGrantType;
import org.iamshield.protocol.oidc.grants.ciba.channel.AuthenticationChannelRequest;
import org.iamshield.protocol.oidc.grants.ciba.channel.AuthenticationChannelResponse;
import org.iamshield.protocol.oidc.grants.ciba.clientpolicy.executor.SecureCibaAuthenticationRequestSigningAlgorithmExecutorFactory;
import org.iamshield.protocol.oidc.grants.ciba.clientpolicy.executor.SecureCibaSessionEnforceExecutorFactory;
import org.iamshield.protocol.oidc.grants.ciba.clientpolicy.executor.SecureCibaSignedAuthenticationRequestExecutor;
import org.iamshield.protocol.oidc.grants.ciba.clientpolicy.executor.SecureCibaSignedAuthenticationRequestExecutorFactory;
import org.iamshield.protocol.oidc.grants.ciba.endpoints.ClientNotificationEndpointRequest;
...
import org.iamshield.services.clientpolicy.executor.ConfidentialClientAcceptExecutorFactory;
...
import org.iamshield.testsuite.rest.representation.TestAuthenticationChannelRequest;
import org.iamshield.testsuite.rest.resource.TestingOIDCEndpointsApplicationResource.AuthorizationEndpointRequestObject;
import org.iamshield.testsuite.services.clientpolicy.executor.TestRaiseExceptionExecutorFactory;
import org.iamshield.testsuite.util.InfinispanTestTimeServiceRule;
...
import org.iamshield.testsuite.util.oauth.ciba.AuthenticationRequestAcknowledgement;
import org.iamshield.testsuite.util.oauth.LogoutResponse;
import org.iamshield.util.JsonSerialization;
import org.iamshield.testsuite.client.policies.AbstractClientPoliciesTest;
...
public class CIBATest extends AbstractClientPoliciesTest {
    ...
    public void addTestRealms(List<RealmRepresentation> testRealms) {
        ...
        ClientRepresentation confApp = IAMShieldModelUtils.createClient(realm, SECOND_TEST_CLIENT_NAME);
...
//            oauth.ciba().doAuthenticationChannelCallback(SECOND_TEST_CLIENT_NAME, SECOND_TEST_CLIENT_SECRET, usernameAuthenticated, authenticationChannelReq.getBearerToken(), SUCCEEDED);
    ...
    public void testSecureCibaSessionEnforceExecutor() throws Exception {
        ...
        json = (new ClientPoliciesBuilder()).addPolicy(
                (new ClientPolicyBuilder()).createPolicy(POLICY_NAME, "La Premiere Politique", Boolean.TRUE)
                    .addCondition(AnyCliConditionFactory.PROVIDER_ID,
    ...
    public void testBackchannelAuthenticationFlowWithInvalidSignedAuthenticationRequest() throws Exception {
        ...
        try {
            ...
            requestObject.audience(Urls.realmIssuer(new URI(suiteContext.getAuthServerInfo().getContextRoot().toString() + "/auth"), REALM_NAME), "https://example.com");
            requestObject.issuer(TEST_CLIENT_NAME);
            requestObject.id(org.iamshield.models.utils.IAMShieldModelUtils.generateId());
            requestObject.iat(Long.valueOf(Time.currentTime()));
    ...
    public void testSecureCibaAuthenticationRequestSigningAlgorithmEnforceExecutor() throws Exception {
                ...
                (new ClientPolicyBuilder()).createPolicy(POLICY_NAME, "Den Forsta Policyn", Boolean.TRUE)
                    .addCondition(CliUpdaterContextConditionFactory.PROVIDER_ID,
                        createClientUpdateContextConditionConfig(Arrays.asList(
                                CliUpdaterContextConditionFactory.BY_AUTHENTICATED_USER,
                                CliUpdaterContextConditionFactory.BY_INITIAL_ACCESS_TOKEN,
                                CliUpdaterContextConditionFactory.BY_REGISTRATION_ACCESS_TOKEN)))
                    .addProfile(PROFILE_NAME)
                    .toRepresentation()
                ).toString();
        updatePolicies(json);
        ...
        String cAppAdminId = createClientByAdmin(generateSuffixedName("App-by-Admin"), (ClientRepresentation clientRep) -> {
                clientRep.setAttributes(new HashMap<>());
                clientRep.getAttributes().put(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG, org.iamshield.crypto.Algorithm.ES256);
            });
        ClientRepresentation cRep = getClientByAdmin(cAppAdminId);
        assertEquals(org.iamshield.crypto.Algorithm.ES256, cRep.getAttributes().get(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG));
        ...
        assertEquals(org.iamshield.crypto.Algorithm.PS256, cRep2.getAttributes().get(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG));

        // update by Admin REST API - fail
        try {
            updateClientByAdmin(cAppAdminId, (ClientRepresentation clientRep) -> {
                clientRep.setAttributes(new HashMap<>());
                clientRep.getAttributes().put(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG, org.iamshield.crypto.Algorithm.RS512);
            });
        ...
        updateClientByAdmin(cAppAdminId, (ClientRepresentation clientRep) -> {
                clientRep.setAttributes(new HashMap<>());
                clientRep.getAttributes().put(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG, org.iamshield.crypto.Algorithm.PS384);
        });
        cRep = getClientByAdmin(cAppAdminId);
        assertEquals(org.iamshield.crypto.Algorithm.PS384, cRep.getAttributes().get(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG));
        ...
        json = (new ClientProfilesBuilder()).addProfile(
                (new ClientProfileBuilder()).createProfile(PROFILE_NAME, "Den Forsta Profilen")
                    .addExecutor(SecureCibaAuthenticationRequestSigningAlgorithmExecutorFactory.PROVIDER_ID,
                            createSecureCibaAuthenticationRequestSigningAlgorithmExecutorConfig(org.iamshield.crypto.Algorithm.ES256))
                    .toRepresentation()
                ).toString();
        ...
        assertEquals(org.iamshield.crypto.Algorithm.ES256, cRep2.getAttributes().get(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG));

        // update profiles, fall back to PS256
        json = (new ClientProfilesBuilder()).addProfile(
                (new ClientProfileBuilder()).createProfile(PROFILE_NAME, "Den Forsta Profilen")
                    .addExecutor(SecureCibaAuthenticationRequestSigningAlgorithmExecutorFactory.PROVIDER_ID,
                            createSecureCibaAuthenticationRequestSigningAlgorithmExecutorConfig(org.iamshield.crypto.Algorithm.RS512))
                    .toRepresentation()
                ).toString();
        updateProfiles(json);
        ...
        try {
            createClientByAdmin(generateSuffixedName("App-in-Dynamic"), (ClientRepresentation clientRep) -> {
                    clientRep.setSecret("secret");
                    clientRep.setAttributes(new HashMap<>());
                    clientRep.getAttributes().put(CibaConfig.CIBA_BACKCHANNEL_AUTH_REQUEST_SIGNING_ALG, org.iamshield.crypto.Algorithm.RS384);
                });
            fail();
        } catch (ClientPolicyException e) {
            assertEquals(OAuthErrorException.INVALID_REQUEST, e.getMessage());
        ...
        String cAppDynamicClientId = createClientDynamically(generateSuffixedName("App-in-Dynamic"), (OIDCClientRepresentation clientRep) -> {
                clientRep.setBackchannelAuthenticationRequestSigningAlg(org.iamshield.crypto.Algorithm.ES256);
            });
        ...
        try {
            updateClientDynamically(cAppDynamicClientId, (OIDCClientRepresentation clientRep) -> {
                     clientRep.setBackchannelAuthenticationRequestSigningAlg(org.iamshield.crypto.Algorithm.RS256);
                 });
            fail();
        } catch (ClientRegistrationException e) {
            assertEquals(ERR_MSG_CLIENT_REG_FAIL, e.getMessage());
        }
        assertEquals(org.iamshield.crypto.Algorithm.ES256, getClientDynamically(cAppDynamicClientId).getBackchannelAuthenticationRequestSigningAlg());

        // update dynamically - success
        updateClientDynamically(cAppDynamicClientId, (OIDCClientRepresentation clientRep) -> {
                clientRep.setBackchannelAuthenticationRequestSigningAlg(org.iamshield.crypto.Algorithm.ES384);
            });
        assertEquals(org.iamshield.crypto.Algorithm.ES384, getClientDynamically(cAppDynamicClientId).getBackchannelAuthenticationRequestSigningAlg());
        ...
        assertEquals(org.iamshield.crypto.Algorithm.PS256, cAppDynamicClient2Rep.getBackchannelAuthenticationRequestSigningAlg());
        ...
        assertEquals(org.iamshield.crypto.Algorithm.ES256, cAppDynamicClient2Rep.getBackchannelAuthenticationRequestSigningAlg());
    }
        ...
        json = (new ClientPoliciesBuilder()).addPolicy(
                (new ClientPolicyBuilder()).createPolicy(POLICY_NAME, "Het Eerste Beleid", Boolean.TRUE)
                    .addCondition(CliScopesConditionFactory.PROVIDER_ID,
                        createClientScopesConditionConfig(CliScopesConditionFactory.OPTIONAL, Arrays.asList("microprofile-jwt")))
                    .addProfile(PROFILE_NAME)
                    .toRepresentation()
                ).toString();
        updatePolicies(json);
