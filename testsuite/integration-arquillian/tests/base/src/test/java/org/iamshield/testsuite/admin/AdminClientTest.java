 ...
 */
public class AdminClientTest extends AbstractIAMShieldTest {
    ...
    public void clientCredentialsAuthSuccess() throws Exception {
        try (IAMShield adminClient = AdminClientUtil.createAdminClientWithClientCredentials(realmName, clientId, clientSecret, null)) {
    ...
    public void adminAuthCloseUserSession() throws Exception {
        ...
        try (IAMShield iamshield = AdminClientUtil.createAdminClient(false, realmName, "test-user@localhost", "password", Constants.ADMIN_CLI_CLIENT_ID, null)) {
            // Check possible to load the realm
            RealmRepresentation realm = iamshield.realm(realmName).toRepresentation();
            Assert.assertEquals(realmName, realm.getRealm());
    ...
    public void adminAuthClientDisabled() throws Exception {
        try (IAMShield adminClient = AdminClientUtil.createAdminClient(false, realmName, "test-user@localhost", "password", Constants.ADMIN_CLI_CLIENT_ID, null)) {
            // Check possible to load the realm
            RealmRepresentation realm = adminClient.realm(realmName).toRepresentation();
            Assert.assertEquals(realmName, realm.getRealm());
    ...
    public void adminAuthUserDisabled() throws Exception {
        try (IAMShield adminClient = AdminClientUtil.createAdminClient(false, realmName, "test-user@localhost", "password", Constants.ADMIN_CLI_CLIENT_ID, null);
             IAMShield adminClientOffline = AdminClientUtil.createAdminClient(false, ServerURLs.getAuthServerContextRoot(), realmName, "test-user@localhost", "password", Constants.ADMIN_CLI_CLIENT_ID, null, OAuth2Constants.OFFLINE_ACCESS, false);
        ) {
            // Check possible to load the realm
            RealmRepresentation realm = adminClient.realm(realmName).toRepresentation();
            Assert.assertEquals(realmName, realm.getRealm());
    ...
    public void scopedClientCredentialsAuthSuccess() throws Exception {
        ...
        String scopeId = createScope(testRealm, scopeName, IAMShieldModelUtils.generateId());
        testRealm.clients().get(clientUUID).addOptionalClientScope(scopeId);

        // with scope
        try (IAMShield adminClient = AdminClientUtil.createAdminClientWithClientCredentials(realmName,
    ...
    public void noClientSecretWithClientCredentialsAuthSuccess() throws Exception {
        ...
        try (IAMShield adminClient = AdminClientUtil.
