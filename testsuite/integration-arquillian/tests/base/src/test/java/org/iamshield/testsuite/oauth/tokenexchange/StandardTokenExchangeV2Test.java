...
package org.iamshield.testsuite.oauth.tokenexchange;
...
import org.iamshield.testsuite.pages.ConsentPage;
...
import org.iamshield.testsuite.updaters.RoleScopeUpdater;
...
import org.iamshield.testsuite.util.oauth.TokenExchangeRequest;
...
public class StandardTokenExchangeV2Test extends AbstractClientPoliciesTest {
    ...
    public void testTransientSessionWithAdminApi() throws Exception {
        ...
        try (RoleScopeUpdater roleScopeUpdater = UserAttributeUpdater.forUserByUsername(realm, "john")
            ...
            try (IAMShield iamshield = IAMShield.getInstance(ServerURLs.getAuthServerContextRoot() + "/auth",
                    TEST, Constants.ADMIN_CLI_CLIENT_ID, response.getAccessToken(), TLSUtils.initializeTLS())) {
                assertEquals(TEST, iamshield.realm(TEST).toRepresentation().getRealm());
                setTimeOffset(10);
                assertEquals(TEST, iamshield.realm(TEST).toRepresentation().getRealm());
                realm.deleteSession(exchangedToken.getSessionId(), false);
                assertThrows(NotAuthorizedException.class, () -> iamshield.realm(TEST).toRepresentation().getRealm());
            }
        }
    }
        ...
        json = (new ClientPoliciesUtil.ClientPoliciesBuilder()).addPolicy(
                (new ClientPoliciesUtil.ClientPolicyBuilder()).createPolicy(POLICY_NAME, "Client Scope Policy", Boolean.TRUE)
                        .addCondition(CliScopesConditionFactory.PROVIDER_ID,
                                createClientScopesConditionConfig(CliScopesConditionFactory.ANY, List.of("optional-scope2")))
