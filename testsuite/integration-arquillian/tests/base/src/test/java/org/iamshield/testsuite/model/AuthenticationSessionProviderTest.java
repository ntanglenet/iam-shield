...
import org.iamshield.models.utils.ResetTimeOffsetEvent;
 ...
 */
public class AuthenticationSessionProviderTest extends AbstractTestRealmIAMShieldTest {
    ...
    @Test
    @ModelTest
    public void testLoginSessionsCRUD(IAMShieldSession session) {
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionCRUD1) -> {
            IAMShieldSession currentSession = sessionCRUD1;
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionCRUD2) -> {
            IAMShieldSession currentSession = sessionCRUD2;
            RealmModel realm = currentSession.realms().getRealm(realmId);
            currentSession.getContext().setRealm(realm);
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionCRUD3) -> {
            IAMShieldSession currentSession = sessionCRUD3;
            RealmModel realm = currentSession.realms().getRealm(realmId);
            currentSession.getContext().setRealm(realm);
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionCRUD4) -> {
            IAMShieldSession currentSession = sessionCRUD4;
            RealmModel realm = currentSession.realms().getRealm(realmId);
            currentSession.getContext().setRealm(realm);
    ...
    @Test
    @ModelTest
    public void testAuthenticationSessionRestart(IAMShieldSession session) {
        AtomicReference<String> parentAuthSessionID = new AtomicReference<>();
        AtomicReference<String> tabID = new AtomicReference<>();
        final int timestamp = Time.currentTime();

        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionRestart1) -> {
            IAMShieldSession currentSession = sessionRestart1;
            RealmModel realm = currentSession.realms().getRealm(realmId);
            currentSession.getContext().setRealm(realm);
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionRestart2) -> {
            IAMShieldSession currentSession = sessionRestart2;
            RealmModel realm = currentSession.realms().getRealm(realmId);
            currentSession.getContext().setRealm(realm);
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionRestart3) -> {
            IAMShieldSession currentSession = sessionRestart3;
            RealmModel realm = currentSession.realms().getRealm(realmId);
            currentSession.getContext().setRealm(realm);
    ...
    @Test
    @ModelTest
    public void testExpiredAuthSessions(IAMShieldSession session) {
        assumeFalse(InfinispanUtils.isRemoteInfinispan());
        AtomicReference<String> authSessionID = new AtomicReference<>();

        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), mainSession -> {
            try {
                ...
                Time.setOffset(0);
                session.getIAMShieldSessionFactory().publish(new ResetTimeOffsetEvent());
    ...
    @Test
    @ModelTest
    public void testOnRealmRemoved(IAMShieldSession session) {
        AtomicReference<String> authSessionID = new AtomicReference<>();
        AtomicReference<String> authSessionID2 = new AtomicReference<>();

        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sesRealmRemoved1) -> {
            IAMShieldSession currentSession = sesRealmRemoved1;
            RealmModel realm = currentSession.realms().getRealm(realmId);
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sesRealmRemoved2) -> {
            IAMShieldSession currentSession = sesRealmRemoved2;
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sesRealmRemoved3) -> {
            IAMShieldSession currentSession = sesRealmRemoved3;
            RealmModel realm = currentSession.realms().getRealm(realmId);
    ...
    @Test
    @ModelTest
    public void testOnClientRemoved(IAMShieldSession session) {
    ...
    private void createAuthSession(IAMShieldSession session, AtomicReference<String> authSessionID) {

        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession createAuthSession) -> {
            IAMShieldSession currentSession = createAuthSession;
            RealmModel realm = currentSession.realms().getRealm(realmId);
    ...
    private void testExpiredOffset(IAMShieldSession session, int offset, boolean isSessionNull, String authSessionID) {

        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionExp) -> {
            IAMShieldSession currentSession = sessionExp;
            RealmModel realm = currentSession.realms().getRealm(realmId);
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionExpVerify) -> {
            IAMShieldSession currentSession = sessionExpVerify;
            RealmModel realm = currentSession.realms().getRealm(realmId);
    ...
    private void setAccessCodeLifespan(IAMShieldSession session, int lifespan, int lifespanUserAction, int lifespanLogin) {

        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession sessionLifespan) -> {
            IAMShieldSession currentSession = sessionLifespan;
            RealmModel realm = currentSession.realms().getRealm(realmId);
