...
public class LDAPGroupMapper2WaySyncTest extends AbstractLDAPTest {
    ...
    public void test01_syncNoPreserveGroupInheritance() throws Exception {
        ...
        testingClient.server().run(session -> {
            ...
            Assert.assertNull(IAMShieldModelUtils.findGroupByPath(session, realm, "/group2"));
        });
        ...
        testingClient.server().run(session -> {
            ...
            GroupModel kcGroup2 = IAMShieldModelUtils.findGroupByPath(session, realm, "/group2");
    ...
    private static void testDropNonExisting(IAMShieldSession session, LDAPTestContext ctx, ComponentModel mapperModel) {
        RealmModel realm = ctx.getRealm();
