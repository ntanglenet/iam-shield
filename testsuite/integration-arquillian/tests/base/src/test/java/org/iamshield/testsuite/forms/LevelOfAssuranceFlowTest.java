...
import org.iamshield.authentication.authenticators.browser.RecoveryAuthnCodesFormAuthenticatorFactory;
import org.iamshield.authentication.authenticators.browser.UsernamePasswordFormFactory;
import org.iamshield.authentication.authenticators.conditional.ConditionalLoaAuthenticator;
import org.iamshield.authentication.authenticators.conditional.ConditionalLoaAuthenticatorFactory;
...
import org.iamshield.models.AuthenticationFlowBindings;
...
import org.iamshield.testsuite.account.AccountRestClient;
...
import org.iamshield.testsuite.authentication.PushButtonAuthenticatorFactory;
...
import org.iamshield.testsuite.pages.PushTheButtonPage;
import org.iamshield.testsuite.pages.SelectAuthenticatorPage;
import org.iamshield.testsuite.pages.SetupRecoveryAuthnCodesPage;
...
import static org.iamshield.testsuite.actions.AppInitiatedActionDeleteCredentialTest.getKcActionParamForDeleteCredential;
...
public class LevelOfAssuranceFlowTest extends AbstractChangeImportedUserPasswordsTest {
    ...
    public static void configureStepUpFlow(IAMShieldTestingClient testingClient) {
        configureStepUpFlow(testingClient, ConditionalLoaAuthenticator.DEFAULT_MAX_AGE, 0, 0);
    }

    private static void configureStepUpFlow(IAMShieldTestingClient testingClient, int maxAge1, int maxAge2, int maxAge3) {
    ...
    private static void configureFlowsWithRecoveryCodes(IAMShieldTestingClient testingClient) {
        testingClient.server(TEST_REALM_NAME)
                .run(session -> {
    ...
    // Backwards compatibility with IAMShield 17 when condition was configured with option "Store Loa in User Session"
    @Test
    public void testBackwardsCompatibilityForLoaConditionConfig() {
        // Reconfigure to the format of IAMShield 17 with option "Store Loa in User Session"
        BrowserFlowTest.revertFlows(testRealm(), FLOW_ALIAS);
        testingClient.server(TEST_REALM_NAME).run(session -> FlowUtil.inCurrentRealm(session).copyBrowserFlow(FLOW_ALIAS));
        testingClient.server(TEST_REALM_NAME)
                .run(session -> FlowUtil.inCurrentRealm(session).selectFlow(FLOW_ALIAS).inForms(forms -> forms.clear()
