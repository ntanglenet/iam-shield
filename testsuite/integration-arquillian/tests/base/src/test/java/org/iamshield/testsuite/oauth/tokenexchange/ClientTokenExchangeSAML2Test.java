...
import static org.iamshield.protocol.saml.SamlProtocol.SAML_ASSERTION_CONSUMER_URL_POST_ATTRIBUTE;
...
public class ClientTokenExchangeSAML2Test extends AbstractIAMShieldTest {
    ...
    public static void setupRealm(IAMShieldSession session) {
    ...
    public void testExchangeToSAML2SignedAssertion() throws Exception {
        ...
        org.iamshield.testsuite.util.oauth.AccessTokenResponse response = oauth.doPasswordGrantRequest("user", "password");
    ...
    public void testBadImpersonator() throws Exception {
        ...
        org.iamshield.testsuite.util.oauth.AccessTokenResponse response = oauth.doPasswordGrantRequest("bad-impersonator", "password");
        String accessToken = response.getAccessToken();
        TokenVerifier<AccessToken> accessTokenVerifier = TokenVerifier.create(accessToken, AccessToken.class);
        AccessToken token = accessTokenVerifier.parse().getToken();
    ...
    private static void addTargetClients(IAMShieldSession session) {
        RealmModel realm = session.realms().getRealmByName(TEST);
    ...
    private static void addDirectExchanger(IAMShieldSession session) {
        RealmModel realm = session.realms().getRealmByName(TEST);
    ...
    private PublicKey publicKeyFromString() {
        ...
        return org.iamshield.testsuite.util.KeyUtils.publicKeyFromString(keyRep.getPublicKey());
    }

    private PrivateKey privateKeyFromString(String privateKey) {
        return org.iamshield.testsuite.util.KeyUtils.privateKeyFromString(privateKey);
    }
}
