...
import org.iamshield.jgroups.certificates.DatabaseJGroupsCertificateProvider;
...
public class JGroupsCertificateRotationClusterTest extends AbstractClusterTest {
    ...
    private static CertificateReloadManager certificateReloadManager(IAMShieldSession session) {
        return GlobalComponentRegistry.componentOf(cacheManager(session), CertificateReloadManager.class);
    }

    private static DatabaseJGroupsCertificateProvider databaseJGroupsCertificateProvider(IAMShieldSession session) {
        return (DatabaseJGroupsCertificateProvider) session.getProvider(JGroupsCertificateProvider.class);
    }

    private static EmbeddedCacheManager cacheManager(IAMShieldSession session) {
        return session.getProvider(InfinispanConnectionProvider.class)
                .getCache(InfinispanConnectionProvider.USER_CACHE_NAME)
                .getCacheManager();
    }

    private static String currentCertificateAlias(IAMShieldSession session) {
        return databaseJGroupsCertificateProvider(session)
                .getCurrentCertificate()
                .getAlias();
    }

    private static void rotateCertificate(IAMShieldSession session) {
