...
public class OIDCJwksClientRegistrationTest extends AbstractClientRegistrationTest {
    ...
    private KeyPair getKeyPairFromGeneratedPems(Map<String, String> generatedKeys) {
        ...
        PrivateKey privateKey = IAMShieldModelUtils.getPrivateKey(privateKeyPem);
        PublicKey publicKey = IAMShieldModelUtils.getPublicKey(publicKeyPem);
    ...
    private String getClientSignedJWT(String clientId, KeyPair keyPair, final String kid) {
        String realmInfoUrl = IAMShieldUriBuilder.fromUri(getAuthServerRoot()).path(ServiceUrlConstants.REALM_INFO_PATH).build(REALM_NAME).toString();
