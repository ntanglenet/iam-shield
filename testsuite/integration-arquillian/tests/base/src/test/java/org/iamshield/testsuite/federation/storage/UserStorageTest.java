...
import org.iamshield.storage.CacheableStorageProviderModel.CachePolicy;
...
import org.iamshield.testsuite.federation.UserMapStorage;
...
import org.iamshield.testsuite.pages.VerifyEmailPage;
...
import org.iamshield.userprofile.DefaultAttributes;
...
import static org.iamshield.models.UserModel.RequiredAction.UPDATE_PROFILE;
import static org.iamshield.storage.UserStorageProviderModel.CACHE_POLICY;
import static org.iamshield.storage.UserStorageProviderModel.EVICTION_DAY;
import static org.iamshield.storage.UserStorageProviderModel.EVICTION_HOUR;
import static org.iamshield.storage.UserStorageProviderModel.EVICTION_MINUTE;
import static org.iamshield.storage.UserStorageProviderModel.IMPORT_ENABLED;
import static org.iamshield.storage.UserStorageProviderModel.MAX_LIFESPAN;
import static org.iamshield.testsuite.actions.RequiredActionEmailVerificationTest.getEmailLink;

import static org.iamshield.testsuite.util.URLAssert.assertCurrentUrlDoesntStartWith;
...
public class UserStorageTest extends AbstractAuthTest {
    ...
    public void afterTestCleanUp() throws URISyntaxException, IOException {
        testingClient.server().run(session -> {
            ...
            UserMapStorageFactory userMapStorageFactory = (UserMapStorageFactory) session.getIAMShieldSessionFactory().getProviderFactory(UserStorageProvider.class, UserMapStorageFactory.PROVIDER_ID);
...
//    @Test
    ...
    @Test
    @ModelTest
    public void testCast(IAMShieldSession session) throws Exception {
        session.getIAMShieldSessionFactory().getProviderFactoriesStream(CredentialProvider.class)
    ...
    @Test
    @ModelTest
    public void testCredentialCRUD(IAMShieldSession session) {
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession currentSession) -> {
            ...
            org.iamshield.testsuite.Assert.assertEquals(0, credentials.count());
        ...
        IAMShieldModelUtils.runJobInTransaction(session.getIAMShieldSessionFactory(), (IAMShieldSession currentSession) -> {
            ...
            org.iamshield.testsuite.Assert.assertTrue(user.credentialManager().moveStoredCredentialTo(otp2Id.get(), null));
        });
    ...
    private void assertOrder(List<CredentialModel> creds, String... expectedIds) {
        org.iamshield.testsuite.Assert.assertEquals(expectedIds.length, creds.size());

        if (creds.size() == 0) return;

        for (int i=0 ; i<expectedIds.length ; i++) {
            org.iamshield.testsuite.Assert.assertEquals(creds.get(i).getId(), expectedIds[i]);
        }
    }
