 ...
 * to the same {@code IAMShield} role.
 * <p/>
 * This class aims to test the fix for {@code KEYCLOAK-8730}. When configuring two or more mappers that map different IDP
 * attributes to the same {@code IAMShield} role, the user would sometimes not be granted the expected {@code IAMShield} role
 ...
 * </ul>
 * This test configures three different OIDC claim mappers that all map to the same {@code IAMShield} role. Only the first
...
public class OidcMultipleClaimToRoleMappersTest extends OidcClaimToRoleMapperTest {
    ...
    protected void createClaimToRoleMapper(String claimValue, IdentityProviderMapperSyncMode syncMode,
        ...
        secondOidcClaimToRoleMapper.setIdentityProviderMapper(ExternalIAMShieldRoleToRoleMapper.PROVIDER_ID);
