 ...
 * @author mhajas
 */
public class IAMShieldQuarkusServerDeployableContainer extends AbstractQuarkusDeployableContainer implements RemoteContainer {

    private static final int DEFAULT_SHUTDOWN_TIMEOUT_SECONDS = 10;

    private static final Logger log = Logger.getLogger(IAMShieldQuarkusServerDeployableContainer.class);
    ...
    private void importRealm() throws IOException, URISyntaxException {
        if (suiteContext.get().isAuthServerMigrationEnabled() && configuration.getImportFile() != null) {
            ...
            List<String> storageOptions = storeProvider.getStoreOptionsToIAMShieldConfImport();
    ...
    protected void checkLiveness() {
        if (!container.isAlive()) {
            throw new IllegalStateException("IAMShield unexpectedly died :(");
        }
    }
