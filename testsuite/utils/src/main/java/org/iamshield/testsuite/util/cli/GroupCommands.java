...
public class GroupCommands {
    ...
    public static class Create extends AbstractCommand {
        ...
        protected void doRunCommand(IAMShieldSession session) {
            ...
            BatchTaskRunner.runInBatches(first, count, batchCount, session.getIAMShieldSessionFactory(), this::createGroupsInBatch);

            log.infof("Command finished. All groups from %s to %s created", groupPrefix + first, groupPrefix
                    + (first + count - 1));
        }

        private void createGroupsInBatch(IAMShieldSession session, int first, int count) {
