...
public abstract class AbstractSessionCacheCommand extends AbstractCommand {
    ...
    protected abstract void doRunCacheCommand(IAMShieldSession session, Cache<String, SessionEntityWrapper> cache);
    ...
    public static class PutCommand extends AbstractSessionCacheCommand {
        ...
        @Override
        protected void doRunCacheCommand(IAMShieldSession session, Cache<String, SessionEntityWrapper> cache) {
    ...
    public static class CreateManySessionsCommand extends AbstractSessionCacheCommand {
        ...
        protected void doRunCacheCommand(IAMShieldSession session, Cache<String, SessionEntityWrapper> cache) {
            ...
            BatchTaskRunner.runInBatches(0, count, batchCount, session.getIAMShieldSessionFactory(), (IAMShieldSession batchSession, int firstInIteration, int countInIteration) -> {
                for (int i=0 ; i<countInIteration ; i++) {
                    UserSessionEntity userSession = new UserSessionEntity(IAMShieldModelUtils.generateId());
