...
public class UserCommands {
    ...
    public static class Create extends AbstractCommand {
        ...
        protected void doRunCommand(IAMShieldSession session) {
            ...
            BatchTaskRunner.runInBatches(first, count, batchCount, session.getIAMShieldSessionFactory(), this::createUsersInBatch);

            log.infof("Command finished. All users from %s to %s created", usernamePrefix + first, usernamePrefix + (first + count - 1));
        }

        private void createUsersInBatch(IAMShieldSession session, int first, int count) {
