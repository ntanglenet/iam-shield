...
import org.iamshield.it.utils.OutputConsumer;
...
public class DistributionIAMShieldServer implements IAMShieldServer {
    ...
    private RawIAMShieldDistribution keycloak;

    private final boolean debug;

    public DistributionIAMShieldServer(boolean debug) {
        this.debug = debug;
    }

    @Override
    public void start(IAMShieldServerConfigBuilder keycloakServerConfigBuilder) {
        keycloak = new RawIAMShieldDistribution(false, MANUAL_STOP, ENABLE_TLS, RE_CREATE, REMOVE_BUILD_OPTIONS_AFTER_BUILD, REQUEST_PORT, new LoggingOutputConsumer());

        // RawIAMShieldDistribution sets "DEBUG_SUSPEND", not "DEBUG" when debug is passed to constructor
        if (debug) {
