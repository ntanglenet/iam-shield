...
public class AdminClientBuilder {
    ...
    private final IAMShieldBuilder delegate;
    private boolean close = false;

    public AdminClientBuilder(AdminClientFactory adminClientFactory, IAMShieldBuilder delegate) {
    ...
    public IAMShield build() {
        IAMShield keycloak = delegate.build();
        if (close) {
