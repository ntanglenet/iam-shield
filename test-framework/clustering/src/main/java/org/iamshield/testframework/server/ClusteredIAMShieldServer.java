...
import org.iamshield.testframework.infinispan.CacheType;
import org.iamshield.testframework.logging.JBossLogConsumer;
import org.testcontainers.images.RemoteDockerImage;
import org.testcontainers.utility.DockerImageName;
import org.testcontainers.utility.LazyFuture;

public class ClusteredIAMShieldServer implements IAMShieldServer {
    ...
    private final DockerIAMShieldDistribution[] containers;
    private final String images;

    private static LazyFuture<String> defaultImage() {
        return DockerIAMShieldDistribution.createImage(true);
    }

    public ClusteredIAMShieldServer(int mumServers, String images) {
        containers = new DockerIAMShieldDistribution[mumServers];
        this.images = images;
    }

    @Override
    public void start(IAMShieldServerConfigBuilder configBuilder) {
        ...
        try {
            clusterLatch.await((long) numServers * DockerIAMShieldDistribution.STARTUP_TIMEOUT_SECONDS, TimeUnit.SECONDS);
    ...
    private void startContainersWithMixedImage(IAMShieldServerConfigBuilder configBuilder, String[] imagePeServer, CountdownLatchLoggingConsumer clusterLatch) {
        ...
        for (int i = 0; i < containers.length; ++i) {
            ...
            var container = new DockerIAMShieldDistribution(false, MANUAL_STOP, REQUEST_PORT, exposedPorts, resolvedImage);
    ...
    private void startContainersWithSameImage(IAMShieldServerConfigBuilder configBuilder, String image, CountdownLatchLoggingConsumer clusterLatch) {
        int[] exposedPorts = new int[]{REQUEST_PORT, MANAGEMENT_PORT};
        LazyFuture<String> imageFuture = image == null || SNAPSHOT_IMAGE.equals(image) ?
                defaultImage() :
                new RemoteDockerImage(DockerImageName.parse(image));
        for (int i = 0; i < containers.length; ++i) {
            var container = new DockerIAMShieldDistribution(false, MANUAL_STOP, REQUEST_PORT, exposedPorts, imageFuture);
            containers[i] = container;
    ...
    private static void configureLogConsumers(DockerIAMShieldDistribution container, int index, CountdownLatchLoggingConsumer clusterLatch) {
    ...
    private void copyProvidersAndConfigs(DockerIAMShieldDistribution container, IAMShieldServerConfigBuilder configBuilder) {
    ...
    public void stop() {
        Arrays.stream(containers)
                ...
                .forEach(DockerIAMShieldDistribution::stop);
    }
