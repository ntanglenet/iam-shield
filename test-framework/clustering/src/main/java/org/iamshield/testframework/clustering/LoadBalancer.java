...
public class LoadBalancer {
    ...
    private final ClusteredIAMShieldServer server;
    private final HashMap<Integer, Origin> urls = new HashMap<>();
    private final Vertx vertx;
    private final HttpProxy proxy;

    public LoadBalancer(ClusteredIAMShieldServer server) {
        this.server = server;
    ...
    public IAMShieldUrls nodeUrls(int index) {
    ...
    private Origin origin(int index) {
        ...
        return urls.computeIfAbsent(index, i ->
              new Origin("localhost", server.getBasePort(i), new IAMShieldUrls(server.getBaseUrl(i), server.getManagementBaseUrl(i)))
        );
    }
    ...
    record Origin(String host, int port, IAMShieldUrls urls) {
    }
}
