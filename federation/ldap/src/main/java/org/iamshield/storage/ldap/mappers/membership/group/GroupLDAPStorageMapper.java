...
package org.iamshield.storage.ldap.mappers.membership.group;
...
import org.iamshield.storage.ldap.mappers.AbstractLDAPStorageMapper;
import org.iamshield.storage.ldap.mappers.membership.CommonLDAPGroupMapper;
import org.iamshield.storage.ldap.mappers.membership.CommonLDAPGroupMapperConfig;
...
import org.iamshield.storage.ldap.mappers.membership.UserRolesRetrieveStrategy;
...
public class GroupLDAPStorageMapper extends AbstractLDAPStorageMapper implements CommonLDAPGroupMapper {
    ...
    private void syncExistingGroup(RealmModel realm, GroupModel kcExistingGroup, Map.Entry<String, LDAPObject> groupEntry,
        ...
        try {
            ...
            IAMShieldModelUtils.runJobInTransaction(ldapProvider.getSession().getIAMShieldSessionFactory(), session -> {
    ...
    protected GroupModel getKcGroupsPathGroup(RealmModel realm) {
        return config.isTopLevelGroupsPath() ? null : IAMShieldModelUtils.findGroupByPath(getSession(), realm, config.getGroupsPath());
    }
    ...
    protected boolean isGroupInGroupPath(RealmModel realm, GroupModel group) {
        ...
        GroupModel groupPathGroup = IAMShieldModelUtils.findGroupByPath(getSession(), realm, config.getGroupsPath());
