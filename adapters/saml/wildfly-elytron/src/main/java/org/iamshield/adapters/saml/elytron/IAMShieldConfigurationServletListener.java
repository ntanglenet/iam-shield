...
import org.iamshield.adapters.saml.DefaultSamlDeployment;
import org.iamshield.adapters.saml.SamlConfigResolver;
...
import org.iamshield.adapters.saml.elytron.infinispan.InfinispanSessionCacheIdMapperUpdater;
...
/**
 * <p>A {@link ServletContextListener} that parses the IAMShield adapter configuration and set the same configuration
 * as a {@link ServletContext} attribute in order to provide to {@link IAMShieldHttpServerAuthenticationMechanism} a way
 ...
 */
public class IAMShieldConfigurationServletListener implements ServletContextListener {

    protected static Logger log = Logger.getLogger(IAMShieldConfigurationServletListener.class);
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        String configResolverClass = servletContext.getInitParameter("iamshield.config.resolver");
        ...
        if (deploymentContext == null) {
            if (configResolverClass != null) {
                try {
                    ...
                    log.infov("Using {0} to resolve IAMShield configuration on a per-request basis.", configResolverClass);
                } catch (Exception ex) {
                    log.errorv("The specified resolver {0} could NOT be loaded. IAMShield is unconfigured and will deny all requests. Reason: {1}", new Object[] { configResolverClass, ex.getMessage() });
                ...
                if (is == null) {
                    log.warn("No adapter configuration.  IAMShield is unconfigured and will deny all requests.");
                ...
                log.debug("IAMShield is using a per-deployment configuration.");
            }
        }
    ...
    private static InputStream getConfigInputStream(ServletContext context) {
        ...
        if (is == null) {
            String path = context.getInitParameter("iamshield.config.file");
            if (path == null) {
                log.debug("using /WEB-INF/iamshield-saml.xml");
                is = context.getResourceAsStream("/WEB-INF/iamshield-saml.xml");
            } else {
                try {
    ...
    public void addTokenStoreUpdaters(ServletContext servletContext) {
        ...
        try {
            String idMapperSessionUpdaterClasses = servletContext.getInitParameter("iamshield.sessionIdMapperUpdater.classes");
            ...
            for (String clazz : idMapperSessionUpdaterClasses.split("\\s*,\\s*")) {
                if (! clazz.isEmpty()) {
                    if (Objects.equals("org.iamshield.adapters.saml.wildfly.infinispan.InfinispanSessionCacheIdMapperUpdater", clazz)) {
