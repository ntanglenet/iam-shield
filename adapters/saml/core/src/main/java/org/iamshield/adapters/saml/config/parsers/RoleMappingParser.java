 ...
 * @version $Revision: 1 $
 */
public class RoleMappingParser extends AbstractIAMShieldSamlAdapterV1Parser<Set<String>> {

    private static final RoleMappingParser INSTANCE = new RoleMappingParser();

    private RoleMappingParser() {
        super(IAMShieldSamlAdapterV1QNames.ROLE_IDENTIFIERS);
    }
    ...
    @Override
    protected void processSubElement(XMLEventReader xmlEventReader, Set<String> target, IAMShieldSamlAdapterV1QNames element, StartElement elementDetail) throws ParsingException {
        switch (element) {
            case ATTRIBUTE:
                target.add(StaxParserUtil.getRequiredAttributeValueRP(elementDetail, IAMShieldSamlAdapterV1QNames.ATTR_NAME));
                break;
        }
    }
}
