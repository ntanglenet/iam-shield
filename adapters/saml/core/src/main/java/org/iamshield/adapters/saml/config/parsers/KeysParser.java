 ...
 * @version $Revision: 1 $
 */
public class KeysParser extends AbstractIAMShieldSamlAdapterV1Parser<List<Key>> {

    private static final KeysParser INSTANCE = new KeysParser();

    private KeysParser() {
        super(IAMShieldSamlAdapterV1QNames.KEYS);
    }
    ...
    @Override
    protected void processSubElement(XMLEventReader xmlEventReader, List<Key> target, IAMShieldSamlAdapterV1QNames element, StartElement elementDetail) throws ParsingException {
        switch (element) {
            case KEY:
