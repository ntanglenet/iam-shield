...
const LOCAL_DIST_NAME = "iamshield-999.0.0-SNAPSHOT.tar.gz";
...
async function startServer() {
  let { scriptArgs, iamshieldArgs } = handleArgs(process.argv.slice(2));
...
function handleArgs(args) {
  ...
  // we need to remove the args that belong to the script so that we can pass the rest through to iamshield
  tokens
  ...
  return { scriptArgs: values, iamshieldArgs: args };
}
...
async function getNightlyAsset() {
  ...
  return release.data.assets.find(
    ({ name }) => name === "iamshield-999.0.0-SNAPSHOT.tar.gz",
  );
}
