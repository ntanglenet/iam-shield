...
import type {
  Clients,
  PolicyQuery,
} from "@iamshield/iamshield-admin-client/lib/resources/clients";
import {
  IamshieldSelect,
  SelectVariant,
  useFetch,
...
export const ResourcesPolicySelect = ({
  ...
  useFetch(
    ...
    ([providers, ...policies]) => {
      setPolicyProviders(
        providers.filter((p: PolicyProviderRepresentation) => p.type !== "resource" && p.type !== "scope"),
      );
      setItems(
        policies
          .flat()
          ...
          .map(convert)
          .filter(
            (item, index: number, self: Policies[]) =>
              index === self.findIndex((otherItem) => item.id === otherItem.id),
          ),
      );
    },
    [search],
