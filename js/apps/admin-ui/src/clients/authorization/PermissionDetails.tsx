...
import { DecisionStrategy } from "@iamshield/iamshield-admin-client/lib/defs/policyRepresentation";
import {
  FormErrorText,
  HelpItem,
  SelectVariant,
...
export default function PermissionDetails() {
  ...
  useFetch(
    async () => {
      if (!permissionId) {
        throw new Error("No permission ID provided");
      }
      ...
      return {
        permission,
        resources: resources.map((r: { _id: string }) => r._id),
        policies: policies.map((p) => p.id!),
        scopes: scopes.map((s: { id: string }) => s.id!),
      };
    },
    ({
      permission,
