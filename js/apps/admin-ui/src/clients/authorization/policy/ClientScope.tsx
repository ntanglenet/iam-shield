...
import { HelpItem, useFetch } from "@iamshield/iamshield-ui-shared";
...
export const ClientScope = () => {
  ...
  useFetch<ClientScopeRepresentation[]>(
    () => adminClient.clientScopes.find(),
    (allScopes = []) => {
      const clientScopes = getValues("clientScopes") || [];
      setSelectedScopes(
        clientScopes.map((s) => allScopes.find((c) => c.id === s.id)!),
      );
            ...
            {selectedScopes.map((scope, index) => (
              <Tr key={scope.id}>
                ...
                <Td>
                  <Button
                    variant="link"
                    className="iamshield__client-authorization__policy-row-remove"
