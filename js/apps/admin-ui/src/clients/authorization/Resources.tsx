...
import type ResourceServerRepresentation from "@iamshield/iamshield-admin-client/lib/defs/resourceServerRepresentation";
import {
  ListEmptyState,
...
export const AuthorizationResources = ({
  ...
  useFetch<ResourceRepresentation[]>(
    () => {
      const params = {
        first,
        max: max + 1,
    ...
    (fetchedResources) =>
      setResources(
        fetchedResources.map((resource: ResourceRepresentation) => ({ ...resource, isExpanded: false })),
      ),
        ...
        {permissions?.length && (
          ...
          >
            <p className="pf-v5-u-pt-xs">
              {permissions.map((permission: ResourceServerRepresentation) => (
