...
import type CredentialRepresentation from "@iamshield/iamshield-admin-client/lib/defs/credentialRepresentation";
import {
  HelpItem,
  SelectControl,
  useAlerts,
...
export const Credentials = ({ client, save, refresh }: CredentialsProps) => {
  ...
  const providerProperties = useMemo(
    () =>
      componentTypes?.["org.iamshield.authentication.ClientAuthenticator"]?.find(
        (p: AuthenticationProviderRepresentation) => p.id === clientAuthenticatorType,
      )?.clientProperties,
