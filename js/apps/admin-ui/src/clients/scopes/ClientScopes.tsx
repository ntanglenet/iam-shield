import type ClientScopeRepresentation from "@iamshield/iamshield-admin-client/lib/defs/clientScopeRepresentation";
...
import { Action, IamshieldDataTable } from "@iamshield/iamshield-ui-shared";
...
export const ClientScopes = ({
  ...
  const loader = async (first?: number, max?: number, search?: string) => {
    ...
    const find = (id: string) =>
      clientScopes.find((clientScope: ClientScopeRepresentation) => id === clientScope.id);

    const optional = optionalClientScopes.map((c: ClientScopeRepresentation) => {
    ...
    const defaultScopes = defaultClientScopes.map((c: ClientScopeRepresentation) => {
      const scope = find(c.id!);
      const row: Row = {
        ...c,
    ...
    const names = rows.map((row: Row) => row.name);

    setRest(
      clientScopes
        .filter((scope: ClientScopeRepresentation) => !names.includes(scope.name))
        .filter((scope: ClientScopeRepresentation) => scope.protocol === protocol),
    );
  ...
  return (
    <>
      ...
      <IamshieldDataTable
        ...
        onSelect={(rows: Row[]) => setSelectedRows([...rows])}
        searchTypeComponent={
          <SearchDropdown
