import type IdentityProviderMapperRepresentation from "@iamshield/iamshield-admin-client/lib/defs/identityProviderMapperRepresentation";
...
const Header = ({ onChange, value, save, toggleDeleteDialog }: HeaderProps) => {
  ...
  useFetch(
    ...
    (fetchedProvider: IdentityProviderRepresentation | undefined) => {
      if (!fetchedProvider) {
...
export default function DetailSettings() {
  ...
  const providerInfo = useMemo(() => {
    const namespaces = [
      "org.iamshield.broker.social.SocialIdentityProvider",
      "org.iamshield.broker.provider.IdentityProvider",
    ];
  ...
  const loader = async () => {
    ...
    type MapperTypeDef = IdentityProviderMapperRepresentation & {
      id?: string;
      name?: string;
    };
    ...
    ])) as [MapperSummary[], Record<string, MapperTypeDef>];

    const components = loaderMappers.map((loaderMapper: MapperSummary) => {
      const mapperType = Object.values(loaderMapperTypes).find(
        (loaderMapperType: MapperTypeDef) =>
          loaderMapper.identityProviderMapper! === loaderMapperType.id!,
      );

      const result: IdPWithMapperAttributes = {
        ...(mapperType || ({} as MapperTypeDef)),
