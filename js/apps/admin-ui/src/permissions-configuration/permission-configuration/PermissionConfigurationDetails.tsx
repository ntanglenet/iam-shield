...
import { useAlerts, useFetch } from "@iamshield/iamshield-ui-shared";
import {
  ActionGroup,
  AlertVariant,
  Button,
...
import ResourceRepresentation from "@iamshield/iamshield-admin-client/lib/defs/resourceRepresentation";
import ScopeRepresentation from "@iamshield/iamshield-admin-client/lib/defs/scopeRepresentation";
...
export default function PermissionConfigurationDetails() {
  ...
  type ResourceTypeItem = { type: string; scopes?: ScopeRepresentation[] };
  ...
  const resourceTypeScopes = useMemo(
    () =>
      resourceTypes
        .filter(({ type }: { type: string }) => type === resourceType)
        .flatMap(({ scopes = [] }) => scopes)
        .map((scope: ScopeRepresentation) => scope?.name || ""),
  ...
  useFetch(
    ...
    ({ providers, policies }) => {
      const filteredProviders = providers?.filter(
        (p: PolicyProviderRepresentation) => p.type !== "resource" && p.type !== "scope",
      );
  ...
  useFetch(
    ...
    ({ permission, resources, policies, scopes }) => {
      const resourceIds = resources?.map((resource: ResourceRepresentation) => resource.name!) || [];
      const policyIds = policies?.map((policy: PolicyRepresentation ) => policy.id!) || [];
      const scopeNames = scopes?.map((scope: ScopeRepresentation) => scope.name).filter((name): name is string => name !== undefined) || [];
