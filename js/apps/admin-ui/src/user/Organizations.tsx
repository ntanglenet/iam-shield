...
export const Organizations = ({ user }: OrganizationProps) => {
  ...
  useFetch(
    async () => {
      ...
      const userOrganizationsWithMembershipTypes = await Promise.all(
        userOrganizations.map(async (org: OrganizationRepresentation) => {
          const orgId = org.id;
          ...
          const membershipType = userMemberships.map((membership) => {
            ...
            return formattedMembershipType as string;
          });
      ...
      if (filteredMembershipTypes.length > 0) {
        filteredOrgs = filteredOrgs.filter((org) =>
          (org as any).membershipType?.some((type: string) =>
      ...
      if (searchTriggerText) {
        filteredOrgs = filteredOrgs.filter((org) =>
          (org as any).name?.toLowerCase().includes(searchTriggerText.toLowerCase()),
        );
      }
  ...
  const [toggleDeleteDialog, DeleteConfirm] = useConfirmDialog({
    ...
    onConfirm: async () => {
      try {
        await Promise.all(
          selectedOrgs.map((org: OrganizationRepresentation) =>
