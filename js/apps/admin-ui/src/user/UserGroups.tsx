import type GroupRepresentation from "@iamshield/iamshield-admin-client/lib/defs/groupRepresentation";
import type UserRepresentation from "@iamshield/iamshield-admin-client/lib/defs/userRepresentation";
import { useHelp } from "@iamshield/iamshield-ui-shared";
import {
  AlertVariant,
  Button,
  ButtonVariant,
...
export const UserGroups = ({ user }: UserGroupsProps) => {
  ...
  const loader = async (first?: number, max?: number, search?: string) => {
    ...
    if (!isDirectMembership)
      joinedUserGroups.forEach((g: GroupRepresentation) => {
        const paths = (
  ...
  return (
    <>
      ...
      <IamshieldDataTable
        key={key}
        loader={loader}
        className="iamshield_user-section_groups-table"
        isPaginated
        ariaLabelKey="roleList"
        searchPlaceholderKey="searchGroup"
        canSelectAll
        onSelect={(groups: GroupRepresentation[]) =>
          isDirectMembership
            ? setSelectedGroups(groups)
            : setSelectedGroups(
                intersectionBy(groups, directMembershipList, "id") as GroupRepresentation[],
              )
        }
        isRowDisabled={(group) =>
          !isDirectMembership &&
          directMembershipList.every((item: GroupRepresentation) => item.id !== group.id)
        }
        toolbarItem={
          <>
            <Button
        ...
        columns={[
...

          {
            ...
            cellRenderer: (group: GroupRepresentation) => {
              const canLeaveGroup =
                directMembershipList.some((item: GroupRepresentation  ) => item.id === group.id) ||
