import type FederatedIdentityRepresentation from "@iamshield/iamshield-admin-client/lib/defs/federatedIdentityRepresentation";
import type IdentityProviderRepresentation from "@iamshield/iamshield-admin-client/lib/defs/identityProviderRepresentation";
import type { IdentityProvidersQuery } from "@iamshield/iamshield-admin-client/lib/resources/identityProviders";
import {
  AlertVariant,
  Button,
  ButtonVariant,
...
import { FormPanel } from "@iamshield/iamshield-ui-shared";
...
export const UserIdentityProviderLinks = ({
    ...
    if (canQueryIDPDetails) {
      ...
      for (const element of allFedIds) {
        element.providerId = allProviders.find(
          (item: IdentityProviderRepresentation) =>
            item.alias === element.identityProvider,
        )?.providerId!;
      }
    }
  ...
  const badgeRenderer1 = (idp: FederatedIdentityRepresentation) => {
    const groupName = identityProviders?.find(
      (provider: IdentityProviderRepresentation) => provider.alias === idp.identityProvider,
    )?.groupName!;
    return (
  ...
  const badgeRenderer2 = (idp: IdentityProviderRepresentation) => {
    const groupName = identityProviders?.find(
      (provider: IdentityProviderRepresentation) => provider.providerId === idp.providerId,
    )?.groupName!;
    return (
