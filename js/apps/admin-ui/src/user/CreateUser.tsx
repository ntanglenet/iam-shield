...
import type { UserProfileMetadata } from "@iamshield/iamshield-admin-client/lib/defs/userProfileMetadata";
import {
  isUserProfileError,
  ...
  const save = async (data: UserFormFields) => {
    try {
      const createdUser = await adminClient.users.create({
        ...
        groups: addedGroups.map((group: GroupRepresentation) => group.path!),
        enabled: true,
      });
      ...
      if (isUserProfileError(error)) {
        setUserProfileServerError(error, form.setError, ((key: string, param: any) =>
          t(key, param)) as TFunction);
      } else {
