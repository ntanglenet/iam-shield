import type PasswordPolicyTypeRepresentation from "@iamshield/iamshield-admin-client/lib/defs/passwordPolicyTypeRepresentation";
import type RealmRepresentation from "@iamshield/iamshield-admin-client/lib/defs/realmRepresentation";
import {
  ActionGroup,
  AlertVariant,
  Button,
...
const PolicySelect = ({ onSelect, selectedPolicies }: PolicySelectProps) => {
  ...
  const policies = useMemo(
    () =>
      passwordPolicies?.filter(
        (p: PasswordPolicyTypeRepresentation) => selectedPolicies.find((o) => o.id === p.id) === undefined,
      ),
    [selectedPolicies],
  );
  ...
  return (
    ...
    >
      <SelectList>
        {policies?.map((policy: PasswordPolicyTypeRepresentation) => (
      ...
      {(rows.length !== 0 || realm.passwordPolicy) && (
        <>
          ...
          <PageSection variant="light">
            <FormProvider {...form}>
              <FormAccess
                className="iamshield__policies_authentication__form"
                role="manage-realm"
                isHorizontal
