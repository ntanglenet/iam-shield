import type TestLdapConnectionRepresentation from "@iamshield/iamshield-admin-client/lib/defs/testLdapConnection";
import {
  HelpItem,
  IamshieldSelect,
  PasswordControl,
...
export const convertFormToSettings = (form: UseFormReturn) => {
  ...
  testLdapProperties.forEach((key) => {
    const value = get(form.getValues(), `config.${key as string}`);
