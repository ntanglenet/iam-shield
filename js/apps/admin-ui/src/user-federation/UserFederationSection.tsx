...
import { ClickableCard } from "../components/Iamshield-card/ClickableCard";
import { IamshieldCard } from "../components/Iamshield-card/IamshieldCard";
...
export default function UserFederationSection() {
  ...
  const providers =
    useServerInfo().componentTypes?.[
      "org.iamshield.storage.UserStorageProvider"
    ] || [];
  ...
  useFetch(
    async () => {
      const testParams: { [name: string]: string | number } = {
        ...
        type: "org.iamshield.storage.UserStorageProvider",
      };
  ...
  if (userFederations) {
    cards = userFederations.sort(cardSorter).map((userFederation, index) => (
      <GalleryItem
        key={index}
        className="iamshield-admin--user-federation__gallery-item"
      >
        <IamshieldCard
