...
export const GroupsModal = ({
  ...
  useFetch(
    ...
    (group: GroupRepresentation | undefined) => {
      if (group) {
    ...
    try {
      ...
      for (const client of clients) {
        ...
        const clientRoles = roles
          .filter((role: RoleRepresentation) => role.id && role.name)
  ...
  const duplicateGroup = async (
    ...
    try {
      ...
      const realmRolesPayload: RoleMappingPayload[] = realmRoles.map(
        (role: RoleRepresentation) => ({ id: role.id!, name: role.name! }),
      );
      ...
      const clientRolesPayload: RoleMappingPayload[] =
        clientRoleMappings?.flatMap((clientRoleMapping) =>
          clientRoleMapping.roles.map((role: RoleRepresentation) => ({
            id: role.id!,
            name: role.name!,
