...
export const GroupTree = ({
  ...
  const mapGroup = (
    ...
    return {
      ...
      defaultExpanded: subGroups.map((g: GroupRepresentation) => g.id).includes(group.id),
    };
  };
  ...
  const nav = (item: TreeViewDataItem, data: ExtendedTreeViewDataItem[]) => {
    ...
    if (!subGroups.every(({ id }: GroupRepresentation) => path.find((t) => t.id === id))) clear();
    if (
      canViewDetails ||
    ...
    >
      {data.length > 0 && (
        <TreeView
          ...
          hasGuides
          hasSelectableNodes
          className="iamshield_groups_treeview"
          onExpand={(_, item) => {
            nav(item, data);
          }}
          onSelect={(_, item) => {
