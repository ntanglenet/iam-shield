...
import { SubGroupQuery } from "@iamshield/iamshield-admin-client/lib/resources/groups";
import {
  Action,
  IamshieldDataTable,
  ListEmptyState,
...
export const Members = () => {
  ...
  const loader = async (first?: number, max?: number) => {
    ...
    if (includeSubGroup && currentGroup?.subGroupCount && currentGroup.id) {
      ...
      members = uniqBy<UserRepresentation>(members, (member) => member.username);
    }
        ...
        actions={[
          ...(isManager
            ? [
                {
                  title: t("leave"),
                  onRowClick: async (user: UserRepresentation) => {
                    try {
            ...
            : []),
          {
            ...
            onRowClick: (user: UserRepresentation) => {
