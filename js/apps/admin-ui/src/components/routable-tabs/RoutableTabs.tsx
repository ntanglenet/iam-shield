...
export const RoutableTabs = ({
  ...
  type ServerTab = {
    id: string;
    metadata: { path: string; params?: Record<string, string> };
  };
  const tabs = (componentTypes?.[TAB_PROVIDER] || []).map((ct) => ({
    id: ct.id!,
    metadata: { path: ct.metadata?.path || "", params: ct.metadata?.params }
  })) as ServerTab[];
  const isFeatureEnabled = useIsFeatureEnabled();
  const { t } = useTranslation();

  const matchedTabs: Array<ServerTab & { pathname: string }> = tabs
  ...
  return (
    ...
    >
      ...
      {isFeatureEnabled(Feature.DeclarativeUI) &&
        matchedTabs.map((tab: (typeof matchedTabs)[number]) => (
          <DynamicTab key={tab.id} eventKey={tab.pathname} title={t(tab.id)}>
            <PageHandler page={tab as any} providerType={TAB_PROVIDER} />
          </DynamicTab>
        ))}
    </Tabs>
  );
