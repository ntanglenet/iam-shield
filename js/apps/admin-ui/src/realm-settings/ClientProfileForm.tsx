...
import type ClientProfilesRepresentation from "@iamshield/iamshield-admin-client/lib/defs/clientProfilesRepresentation";
import {
  HelpItem,
  TextAreaControl,
  TextControl,
  ...
  useFetch(
    ...
    (profiles: ClientProfilesRepresentation) => {
      setProfiles({
        ...
        profiles: profiles.profiles?.filter((p: ClientProfileRepresentation) => p.name !== profileName),
      });
      const globalProfile = profiles.globalProfiles?.find(
        (p: ClientProfileRepresentation) => p.name === profileName,
      );
      const profile = profiles.profiles?.find((p: ClientProfileRepresentation) => p.name === profileName);
