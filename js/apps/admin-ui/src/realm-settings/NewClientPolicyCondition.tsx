import type { ConfigPropertyRepresentation } from "@iamshield/iamshield-admin-client/lib/defs/authenticatorConfigInfoRepresentation";
import type ClientPolicyConditionRepresentation from "@iamshield/iamshield-admin-client/lib/defs/clientPolicyConditionRepresentation";
import type ClientPolicyRepresentation from "@iamshield/iamshield-admin-client/lib/defs/clientPolicyRepresentation";
import type ComponentTypeRepresentation from "@iamshield/iamshield-admin-client/lib/defs/componentTypeRepresentation";
import {
  HelpItem,
  IamshieldSelect,
  SelectVariant,
...
export default function NewClientPolicyCondition() {
  ...
  const conditionTypes =
    serverInfo.componentTypes?.[
      "org.iamshield.services.clientpolicy.condition.ClientPolicyConditionProvider"
    ];
  ...
  useFetch(
    ...
    (
      ...
      if (conditionName) {
        let currentPolicy = response.policies?.find(
          (item: ClientPolicyRepresentation) => item.name === policyName,
        );
        if (currentPolicy === undefined) {
          currentPolicy = response.globalPolicies?.find(
            (item: ClientPolicyRepresentation) => item.name === policyName,
          );
        ...
        const typeAndConfigData = currentPolicy?.conditions?.find(
          (item: ClientPolicyConditionRepresentation) => item.condition === conditionName,
        );

        const currentCondition = conditionTypes?.find(
          (condition: ComponentTypeRepresentation) => condition.id === conditionName,
        );
  ...
  const save = async (configPolicy: ConfigProperty) => {
    ...
    const writeConfig = () => {
      return conditionProperties.reduce((r: any, p: ConfigPropertyRepresentation) => {
        r[p.name!] = configValues[p.name!];
        return r;
      }, {});
    };

    const updatedPolicies = policies.map((policy: ClientPolicyRepresentation) => {
      ...
      if (conditionName) {
        ...
        const index = conditions.findIndex(
          (condition: ClientPolicyConditionRepresentation) => conditionName === condition.condition,
        );
          ...
          >
            <Controller
              ...
              render={({ field }) => (
                ...
                >
                  {conditionTypes?.map((condition: ComponentTypeRepresentation) => (
                    <SelectOption
