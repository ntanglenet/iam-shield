...
import {
  ...
  UserProfileConfig,
} from "@iamshield/iamshield-admin-client/lib/defs/userProfileMetadata";
import {
  FormErrorText,
  HelpItem,
  IamshieldSpinner,
...
export const RealmSettingsGeneralTab = ({
  ...
  useFetch(
    ...
    (config: UserProfileConfig) => setUserProfileConfig(config),
    [],
  );
...
function RealmSettingsGeneralTabForm({
  ...
  const setupForm = () => {
    ...
    const attrs = (realm as unknown as { attributes?: Record<string, string> })
      .attributes;
    if (attrs?.["acr.loa.map"]) {
      const result = Object.entries(
        JSON.parse(attrs["acr.loa.map"]),
        ...
        >
          ...
          >
            <Stack>
              ...
              {isOpenid4vciEnabled && (realm as any)?.verifiableCredentialsEnabled && (
                <StackItem>
                  <FormattedLink
                    href={`${addTrailingSlash(
                      serverBaseUrl,
