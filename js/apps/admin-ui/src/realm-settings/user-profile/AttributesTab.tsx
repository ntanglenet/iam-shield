import type { UserProfileAttribute } from "@iamshield/iamshield-admin-client/lib/defs/userProfileMetadata";
import { IamshieldSelect, SelectVariant } from "@iamshield/iamshield-ui-shared";
import {
  Button,
  ButtonVariant,
  Divider,
...
export const AttributesTab = ({ setTableData }: AttributesTabProps) => {
  ...
  const [toggleDeleteDialog, DeleteConfirm] = useConfirmDialog({
    ...
    onConfirm: async () => {
      ...
      const translationsToDelete = config.attributes.find(
        (attribute: UserProfileAttribute) => attribute.name === attributeToDelete,
      )?.displayName;
  ...
  const executeMove = async (
    ...
    const fromIndex = attributes.findIndex((attr: UserProfileAttribute) => {
    ...
    <>
            ...
            <IamshieldSelect
              ...
              onSelect={(value) => {
                ...
                setData(
                  filter === "allGroups"
                    ? attributes
                    : attributes.filter((attr: UserProfileAttribute) => attr.group === filter),
                );
            ...
            >
              {[
                ...
                ...(
                  uniqBy(
                    attributes.filter(
                      (attr: UserProfileAttribute) => !!attr.group,
                    ),
                    "group",
      ...
      <DraggableTable
        ...
        onDragFinish={async (nameDragged, items) => {
          const keys = attributes.map((e: UserProfileAttribute) => e.name);
