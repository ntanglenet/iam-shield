...
export const RealmOverrides = ({
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (filter) {
          const searchInTranslations = (idx: number) => {
            return Object.entries(result as Record<string, string>).filter(
              ([key, value]) => (idx === 0 ? key : value).includes(filter),
            );
          };
    ...
    void fetchLocalizationTexts().then((translations) => {
      const typedTranslations = translations as [string, string][];
      const updatedRows: IRow[] = typedTranslations.map(
        (translation): IRow => ({
          rowEditBtnAriaLabel: () =>
