 ...
 * @version $Revision: 1 $
 */
public interface IAMShieldSession extends AutoCloseable {

    IAMShieldContext getContext();

    IAMShieldTransactionManager getTransactionManager();

    /**
     * Get dedicated provider instance of provider type clazz that was created for this session.  If one hasn't been created yet,
     * find the factory and allocate by calling ProviderFactory.create(IAMShieldSession).  The provider to use is determined
     ...
     * find the factory and allocate by calling ProviderFactory.create(IAMShieldSession).
     ...
     * The relevant realm must be set prior to calling this method in the context, see {@link IAMShieldContext#getRealm()}.
     * @param <T>
     * @param clazz
     ...
     * @return Provider configured according to the {@param componentId}, {@code null} if it cannot be instantiated.
     */
    <T extends Provider> T getComponentProvider(Class<T> clazz, String componentId, Function<IAMShieldSessionFactory, ComponentModel> modelGetter);
    ...
    IAMShieldSessionFactory getIAMShieldSessionFactory();

    /**
     * Returns a managed provider instance.  Will start a provider transaction.  This transaction is managed by the IAMShieldSession
     * transaction.
     *
     * @return
