 ...
 */
public class IAMShieldFipsSecurityProvider extends Provider {

    protected static final Logger logger = Logger.getLogger(IAMShieldFipsSecurityProvider.class);

    private final BouncyCastleFipsProvider bcFipsProvider;

    public IAMShieldFipsSecurityProvider(BouncyCastleFipsProvider bcFipsProvider) {
        super("KC(" +
    ...
    public static String isSystemFipsEnabled() {
        ...
        try {
            Class<?> securityConfigurator = IAMShieldFipsSecurityProvider.class.getClassLoader().loadClass("java.security.SystemConfigurator");
