...
[source]
----
META-INF/iamshield-scripts.json
...
The `META-INF/iamshield-scripts.json` is a file descriptor that provides metadata information about the scripts you want to deploy. It is a JSON file with the following structure:
...
For each script file in your `JAR` file, you need a corresponding entry in `META-INF/iamshield-scripts.json` that maps your scripts files to a specific provider type. For that you should provide the following properties for each entry:
