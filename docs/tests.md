...
To start a basic Keycloak server for testing run:
    ...
    mvn exec:java -Piamshield-server
    
or run org.iamshield.testsuite.KeycloakServer from your favourite IDE!
     
When starting the server it can also import a realm from a json file:

    mvn exec:java -Piamshield-server -Dimport=testrealm.json
...
and https port, optionally setting the truststore.
    ...
    mvn exec:java -Piamshield-server \
        ...
        -Diamshield.port.https=8443

### Configuration

KeycloakServer can be configured with system properties, or through the file `~/.iamshield-server.properties`. Using the 
...
|Key| Description                                                                        |
|---|------------------------------------------------------------------------------------|
|iamshield.port| Set the HTTP port for the server                                         |
|iamshield.port.https| Set the HTTPS port for the server                                  |
|iamshield.bind.address| Set the bind address for the server                              |
|resources| Loads theme resources directly from the workspace and disables caching       |
|iamshield.theme.dir| Specifies an alternative location to load custom themes from        |
|iamshield.theme.cacheTemplates| Disables theme template caching                          |
|iamshield.theme.cacheThemes| Disables theme caching                                      |
|iamshield.theme.staticMaxAge| Sets the client-side caching max-age for theme resources   |
...
* Property `iamshield.createAdminUser` is set to `true` (defaults to `true` if not present)
...
The Keycloak test server can load resources directly from the filesystem instead of the classpath. This allows editing html, styles and updating images without restarting the server. To make the server use resources from the filesystem start with:
    ...
    mvn exec:java -Piamshield-server -Dresources
    
You can also specify the theme directory used by the server with:

    mvn exec:java -Piamshield-server -Diamshield.theme.dir=<PATH TO THEMES DIR>
    
For example to use the example themes run the server with:

    mvn exec:java -Piamshield-server -Diamshield.theme.dir=examples/themes
    
**NOTE:** If `iamshield.theme.dir` is specified the default themes (base, rcue and iamshield) are loaded from the classpath
...
or run org.iamshield.testsuite.TotpGenerator from your favourite IDE!
...
* User DN Suffix: ou=People,dc=iamshield,dc=org
...
This allows to start 2 kerberos servers in parallel to test things like Kerberos cross-realm trust:
    ...
    mvn exec:java -Pkerberos -Diamshield.kerberos.realm=KC2.COM
...
```
mvn exec:java -Piamshield-server -DstartTestsuiteCLI
```
...
```
mvn exec:java -Piamshield-server -Diamshield.connectionsJpa.url=jdbc:mysql://localhost/iamshield -Diamshield.connectionsJpa.driver=com.mysql.jdbc.Driver -Diamshield.connectionsJpa.user=iamshield -Diamshield.connectionsJpa.password=iamshield -DstartTestsuiteCLI
```
