...
public class BuildAndStartDistTest {
    ...
    @DryRun
    @Test
    void testBuildAndStart(IAMShieldDistribution dist) {
    ...
    @Launch({"start-dev"})
    void testCreateLegacyAdmin(IAMShieldDistribution dist, LaunchResult result) {
    ...
    @Launch({"start-dev"})
    void testCreateAdmin(IAMShieldDistribution dist, LaunchResult result) {
    ...
    @WithEnvVars({"KC_BOOTSTRAP_ADMIN_USERNAME", "admin123", "KC_BOOTSTRAP_ADMIN_PASSWORD", "admin123"})
    @Launch({"start-dev"})
    void testCreateDifferentAdmin(IAMShieldDistribution dist, LaunchResult result) {
        assertAdminCreation(dist, result, "admin123", "new-admin", "new-admin");
    }

    private void assertAdminCreation(IAMShieldDistribution dist, LaunchResult result, String initialUsername, String nextUsername, String password) {
        ...
        CLIResult cliResult = dist.run("start-dev", "--log-level=org.iamshield.services:debug");
