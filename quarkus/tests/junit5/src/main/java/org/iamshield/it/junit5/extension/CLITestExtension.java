...
import org.iamshield.quarkus.runtime.cli.command.StartDev;
...
import org.iamshield.quarkus.runtime.configuration.IAMShieldPropertiesConfigSource;
...
import static org.iamshield.it.junit5.extension.DistributionTest.ReInstall.BEFORE_ALL;
import static org.iamshield.it.junit5.extension.DistributionType.RAW;
import static org.iamshield.quarkus.runtime.Environment.forceTestLaunchMode;
import static org.iamshield.quarkus.runtime.cli.command.Main.CONFIG_FILE_LONG_NAME;
import static org.iamshield.quarkus.runtime.cli.command.Main.CONFIG_FILE_SHORT_NAME;

public class CLITestExtension extends QuarkusMainTestExtension {

    private static final String SYS_PROPS = "sys-props";
    private IAMShieldDistribution dist;
        ...
        if (launch != null && distConfig == null) {
            ConfigArgsConfigSource.parseConfigArgs(List.of(launch.value()), (arg, value) -> {
                if (arg.equals(CONFIG_FILE_SHORT_NAME) || arg.equals(CONFIG_FILE_LONG_NAME)) {
                    setProperty(IAMShieldPropertiesConfigSource.KEYCLOAK_CONFIG_FILE_PROP, value);
    ...
    private void copyTestProvider(TestProvider provider) {
        ...
        if (isRaw()) {
            try {
                dist.unwrap(RawIAMShieldDistribution.class).copyProvider(provider.value().getDeclaredConstructor().newInstance());
    ...
    private IAMShieldDistribution createDistribution(DistributionTest config, Storage storeConfig, WithDatabase databaseConfig) {
        return new IAMShieldDistributionDecorator(storeConfig, databaseConfig, config, DistributionType.getCurrent().orElse(RAW).newInstance(config));
    }
    ...
    public Object resolveParameter(ParameterContext parameterContext, ExtensionContext context)
        ...
        if (type.equals(IAMShieldDistribution.class)) {
    ...
    public boolean supportsParameter(ParameterContext parameterContext, ExtensionContext extensionContext)
            throws ParameterResolutionException {
        Class<?> type = parameterContext.getParameter().getType();
        return type == LaunchResult.class || type == CLIResult.class || type == RawDistRootPath.class || type == IAMShieldDistribution.class;
    }
    ...
    private RawDistRootPath getDistPath(){
        return new RawDistRootPath(dist.unwrap(RawIAMShieldDistribution.class).getDistPath());
    }
}
