...
package org.iamshield.quarkus.runtime.storage.database.liquibase;
...
import org.iamshield.connections.jpa.entityprovider.JpaEntityProvider;
import org.iamshield.connections.jpa.updater.JpaUpdaterProvider;
import org.iamshield.connections.jpa.updater.liquibase.LiquibaseConstants;
import org.iamshield.connections.jpa.updater.liquibase.ThreadLocalSessionContext;
import org.iamshield.connections.jpa.updater.liquibase.conn.CustomChangeLogHistoryService;
import org.iamshield.connections.jpa.updater.liquibase.conn.IAMShieldLiquibase;
import org.iamshield.connections.jpa.updater.liquibase.conn.LiquibaseConnectionProvider;
...
public class QuarkusJpaUpdaterProvider implements JpaUpdaterProvider {
    ...
    public QuarkusJpaUpdaterProvider(IAMShieldSession session) {
    ...
    private void update(Connection connection, File file, String defaultSchema) {
        ...
        try {
            if (needVerifyMasterChangelog()) {
                // Run update with iamshield master changelog first
                IAMShieldLiquibase liquibase = getLiquibaseForKeycloakUpdate(connection, defaultSchema);
                if (file != null) {
            ...
            for (JpaEntityProvider jpaProvider : jpaProviders) {
                ...
                if (customChangelog != null) {
                    ...
                    IAMShieldLiquibase liquibase = getLiquibaseForCustomProviderUpdate(connection, defaultSchema, customChangelog, jpaProvider.getClass().getClassLoader(), changelogTableName);
                    updateChangeSet(liquibase, exportWriter);
                }
            }
    ...
    protected void updateChangeSet(IAMShieldLiquibase liquibase, Writer exportWriter) throws LiquibaseException  {
    ...
    private void outputChangeLogTableCreationScript(Liquibase liquibase, final Writer exportWriter) throws DatabaseException {
        ...
        loggingExecutor.comment("* IAMShield database creation script - apply this script to empty DB *");
        ...
        // in org.iamshield.connections.jpa.updater.liquibase.lock.CustomLockService.init() called indirectly from
    ...
    public Status validate(Connection connection, String defaultSchema) {
        ...
        try {
            if (needVerifyMasterChangelog()) {
                // Validate with iamshield master changelog first
                IAMShieldLiquibase liquibase = getLiquibaseForKeycloakUpdate(connection, defaultSchema);
    ...
    protected Status validateChangeSet(IAMShieldLiquibase liquibase, String changelog) throws LiquibaseException {
        final Status result;
        List<ChangeSet> changeSets = getLiquibaseUnrunChangeSets(liquibase);
    ...
    private void resetLiquibaseServices(IAMShieldLiquibase liquibase) {
    ...
    private IAMShieldLiquibase getLiquibaseForKeycloakUpdate(Connection connection, String defaultSchema) throws LiquibaseException {
        LiquibaseConnectionProvider liquibaseProvider = session.getProvider(LiquibaseConnectionProvider.class);
        return liquibaseProvider.getLiquibase(connection, defaultSchema);
    }

    private IAMShieldLiquibase getLiquibaseForCustomProviderUpdate(Connection connection, String defaultSchema, String changelogLocation, ClassLoader classloader, String changelogTableName) throws LiquibaseException {
        LiquibaseConnectionProvider liquibaseProvider = session.getProvider(LiquibaseConnectionProvider.class);
