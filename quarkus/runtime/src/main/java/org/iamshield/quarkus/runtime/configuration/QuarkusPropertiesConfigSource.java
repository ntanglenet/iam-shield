...
import static org.iamshield.quarkus.runtime.configuration.MicroProfileConfigProvider.NS_QUARKUS;
...
public final class QuarkusPropertiesConfigSource extends AbstractLocationConfigSourceLoader implements ConfigSourceProvider {
    ...
    public synchronized List<ConfigSource> getConfigSources(final ClassLoader classLoader) {
        ...
        if (configFile != null) {
            configSources.addAll(loadConfigSources(configFile.toUri().toString(), IAMShieldPropertiesConfigSource.PROPERTIES_FILE_ORDINAL, classLoader));
        }
