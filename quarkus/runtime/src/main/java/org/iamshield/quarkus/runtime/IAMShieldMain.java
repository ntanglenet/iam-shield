...
import static org.iamshield.quarkus.runtime.Environment.getIAMShieldModeFromProfile;
import static org.iamshield.quarkus.runtime.Environment.isNonServerMode;
import static org.iamshield.quarkus.runtime.Environment.isTestLaunchMode;
...
import org.iamshield.quarkus.runtime.integration.jaxrs.QuarkusIAMShieldApplication;
...
import org.iamshield.quarkus.runtime.cli.PropertyException;
import org.iamshield.quarkus.runtime.cli.ExecutionExceptionHandler;
...
import org.iamshield.quarkus.runtime.cli.command.DryRunMixin;
import org.iamshield.quarkus.runtime.cli.command.Start;
 ...
 */
@QuarkusMain(name = "iamshield")
@ApplicationScoped
public class IAMShieldMain implements QuarkusApplication {
    ...
    public static void start(Picocli picocli, AbstractNonServerCommand command, ExecutionExceptionHandler errorHandler) {
        COMMAND = command; // it would be nice to not do this statically - start quarkus with an instance of IAMShieldMain, rather than a class for example
        try {
            Quarkus.run(IAMShieldMain.class, (exitCode, cause) -> {
                if (cause != null) {
                    errorHandler.error(picocli.getErrWriter(),
                            String.format("Failed to start server in (%s) mode", getIAMShieldModeFromProfile(org.iamshield.common.util.Environment.getProfile())),
                            cause.getCause());
                }
            ...
            errorHandler.error(picocli.getErrWriter(),
                    String.format("Unexpected error when starting the server in (%s) mode", getIAMShieldModeFromProfile(org.iamshield.common.util.Environment.getProfile())),
                    cause.getCause());
        }
    ...
    public int run(String... args) throws Exception {
        if (COMMAND != null) {
            QuarkusIAMShieldApplication application = Arc.container().instance(QuarkusIAMShieldApplication.class).get();
