...
import org.iamshield.quarkus.runtime.configuration.AbstractConfigurationTest;
...
public class PicocliTest extends AbstractConfigurationTest {
    ...
    private class NonRunningPicocli extends Picocli {
        ...
        public void initConfig(List<String> cliArgs, AbstractCommand command) {
            IAMShieldConfigSourceProvider.reload();
            ...
            if (!checkBuild && PersistedConfigSource.getInstance().getConfigValueProperties().isEmpty()) {
                System.getProperties().remove(Environment.IAMSHIELD_CONFIG_REBUILD_CHECK);
            }
    ...
    NonRunningPicocli pseudoLaunch(String... args) {
        ...
        IAMShieldMain.main(args, nonRunningPicocli);
    ...
    public void testReaugFromProdToDev() {
        ...
        assertEquals("dev", nonRunningPicocli.buildProps.getProperty(org.iamshield.common.util.Environment.PROFILE));
    }
    ...
    public void testReaugFromDevToProdExport() {
        ...
        assertEquals(AbstractAutoBuildCommand.REBUILT_EXIT_CODE, nonRunningPicocli.exitCode);
        assertTrue(nonRunningPicocli.reaug);
        assertEquals("prod", nonRunningPicocli.buildProps.getProperty(org.iamshield.common.util.Environment.PROFILE));
    }
    ...
    public void wrongLevelForCategory() {
        NonRunningPicocli nonRunningPicocli = pseudoLaunch("start-dev", "--log-level-org.iamshield=wrong");
        assertEquals(CommandLine.ExitCode.USAGE, nonRunningPicocli.exitCode);
    ...
    public void wildcardLevelForCategory() {
        NonRunningPicocli nonRunningPicocli = pseudoLaunch("start-dev", "--log-level-org.iamshield=warn");
        assertEquals(CommandLine.ExitCode.OK, nonRunningPicocli.exitCode);
        var value = nonRunningPicocli.config.getConfigValue("quarkus.log.category.\"org.iamshield\".level");
        assertEquals("quarkus.log.category.\"org.iamshield\".level", value.getName());
        assertEquals("WARN", value.getValue());
        value = nonRunningPicocli.config.getConfigValue("quarkus.log.category.\"org.iamshield1\".level");
        assertEquals("quarkus.log.category.\"org.iamshield1\".level", value.getName());
