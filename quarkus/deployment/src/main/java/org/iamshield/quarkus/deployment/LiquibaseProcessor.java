...
import org.iamshield.connections.jpa.updater.liquibase.lock.DummyLockService;
...
import static org.iamshield.quarkus.deployment.IAMShieldProcessor.getDefaultDataSource;

class LiquibaseProcessor {

    @Record(ExecutionTime.STATIC_INIT)
    @BuildStep
    void configure(IAMShieldRecorder recorder, List<JdbcDataSourceBuildItem> jdbcDataSources, CombinedIndexBuildItem indexBuildItem) {
    ...
    private void filterImplementations(Class<?> types, String dbKind, Set<ClassInfo> classes) {
        if (Database.class.equals(types)) {
            ...
            classes.removeIf(classInfo -> !org.iamshield.config.database.Database.isLiquibaseDatabaseSupported(classInfo.name().toString(), dbKind));
        }
    }
}
