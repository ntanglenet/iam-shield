...
import org.iamshield.authentication.AuthenticatorSpi;
import org.iamshield.authentication.authenticators.browser.DeployedScriptAuthenticatorFactory;
import org.iamshield.authorization.policy.provider.PolicySpi;
import org.iamshield.authorization.policy.provider.js.DeployedScriptPolicyFactory;
import org.iamshield.common.Profile;
import org.iamshield.common.crypto.FipsMode;
import org.iamshield.common.util.MultiSiteUtils;
import org.iamshield.common.util.StreamUtil;
import org.iamshield.config.DatabaseOptions;
import org.iamshield.config.HealthOptions;
import org.iamshield.config.HttpOptions;
import org.iamshield.config.LoggingOptions;
import org.iamshield.config.ManagementOptions;
...
import org.iamshield.config.TransactionOptions;
import org.iamshield.config.database.Database;
...
import org.iamshield.connections.jpa.updater.liquibase.LiquibaseJpaUpdaterProviderFactory;
import org.iamshield.connections.jpa.updater.liquibase.conn.DefaultLiquibaseConnectionProvider;
import org.iamshield.infinispan.util.InfinispanUtils;
import org.iamshield.policy.BlacklistPasswordPolicyProviderFactory;
import org.iamshield.protocol.ProtocolMapperSpi;
import org.iamshield.protocol.oidc.mappers.DeployedScriptOIDCProtocolMapper;
import org.iamshield.protocol.saml.mappers.DeployedScriptSAMLProtocolMapper;
...
import org.iamshield.quarkus.runtime.IAMShieldRecorder;
import org.iamshield.quarkus.runtime.cli.Picocli;
import org.iamshield.quarkus.runtime.configuration.Configuration;
import org.iamshield.quarkus.runtime.configuration.IAMShieldConfigSourceProvider;
import org.iamshield.quarkus.runtime.configuration.MicroProfileConfigProvider;
import org.iamshield.quarkus.runtime.configuration.PersistedConfigSource;
import org.iamshield.quarkus.runtime.configuration.PropertyMappingInterceptor;
import org.iamshield.quarkus.runtime.configuration.mappers.PropertyMapper;
import org.iamshield.quarkus.runtime.configuration.mappers.PropertyMappers;
import org.iamshield.quarkus.runtime.configuration.mappers.WildcardPropertyMapper;
import org.iamshield.quarkus.runtime.integration.resteasy.IAMShieldHandlerChainCustomizer;
import org.iamshield.quarkus.runtime.integration.resteasy.IAMShieldTracingCustomizer;
import org.iamshield.quarkus.runtime.logging.ClearMappedDiagnosticContextFilter;
import org.iamshield.quarkus.runtime.services.health.IAMShieldClusterReadyHealthCheck;
import org.iamshield.quarkus.runtime.services.health.IAMShieldReadyHealthCheck;
import org.iamshield.quarkus.runtime.storage.database.jpa.NamedJpaConnectionProviderFactory;
import org.iamshield.quarkus.runtime.themes.FlatClasspathThemeResourceProviderFactory;
import org.iamshield.representations.provider.ScriptProviderDescriptor;
import org.iamshield.representations.provider.ScriptProviderMetadata;
...
import org.iamshield.theme.FolderThemeProviderFactory;
import org.iamshield.theme.JarThemeProviderFactory;
import org.iamshield.theme.ThemeResourceSpi;
import org.iamshield.transaction.JBossJtaTransactionManagerLookup;
...
import org.iamshield.vault.FilesKeystoreVaultProviderFactory;
import org.iamshield.vault.FilesPlainTextVaultProviderFactory;
...
import static org.iamshield.config.DatabaseOptions.DB;
import static org.iamshield.connections.jpa.util.JpaUtils.loadSpecificNamedQueries;
import static org.iamshield.quarkus.runtime.Environment.getCurrentOrCreateFeatureProfile;
import static org.iamshield.quarkus.runtime.Providers.getProviderManager;
import static org.iamshield.quarkus.runtime.configuration.Configuration.getOptionalBooleanKcValue;
import static org.iamshield.quarkus.runtime.configuration.Configuration.getOptionalKcValue;
import static org.iamshield.quarkus.runtime.configuration.Configuration.getOptionalValue;
import static org.iamshield.quarkus.runtime.configuration.MicroProfileConfigProvider.NS_IAMSHIELD_PREFIX;
import static org.iamshield.quarkus.runtime.storage.database.jpa.QuarkusJpaConnectionProviderFactory.DEFAULT_PERSISTENCE_UNIT;
import static org.iamshield.quarkus.runtime.storage.database.jpa.QuarkusJpaConnectionProviderFactory.QUERY_PROPERTY_PREFIX;
import static org.iamshield.representations.provider.ScriptProviderDescriptor.AUTHENTICATORS;
import static org.iamshield.representations.provider.ScriptProviderDescriptor.MAPPERS;
import static org.iamshield.representations.provider.ScriptProviderDescriptor.POLICIES;
import static org.iamshield.representations.provider.ScriptProviderDescriptor.SAML_MAPPERS;
import static org.iamshield.theme.ClasspathThemeProviderFactory.IAMSHIELD_THEMES_JSON;

class IAMShieldProcessor {

    private static final Logger logger = Logger.getLogger(IAMShieldProcessor.class);
    ...
    private static final String IAMSHIELD_SCRIPTS_JSON_PATH = "META-INF/iamshield-scripts.json";
    ...
    static {
        DEPLOYEABLE_SCRIPT_PROVIDERS.put(AUTHENTICATORS, IAMShieldProcessor::registerScriptAuthenticator);
        DEPLOYEABLE_SCRIPT_PROVIDERS.put(POLICIES, IAMShieldProcessor::registerScriptPolicy);
        DEPLOYEABLE_SCRIPT_PROVIDERS.put(MAPPERS, IAMShieldProcessor::registerScriptMapper);
        DEPLOYEABLE_SCRIPT_PROVIDERS.put(SAML_MAPPERS, IAMShieldProcessor::registerSAMLScriptMapper);
    }
    ...
    FeatureBuildItem getFeature() {
        return new FeatureBuildItem("iamshield");
    }

    @Record(ExecutionTime.STATIC_INIT)
    @BuildStep
    @Produce(ConfigBuildItem.class)
    void initConfig(IAMShieldRecorder recorder) {
    ...
    void configureProfile(IAMShieldRecorder recorder) {
    ...
    @BuildStep
    @Consume(ConfigBuildItem.class)
    void configureTruststore(IAMShieldRecorder recorder) {
    ...
    @Record(ExecutionTime.STATIC_INIT)
    void setDefaultUserProfileConfig(IAMShieldRecorder recorder, UserProfileBuildItem configuration) {
    ...
    void checkPersistenceUnits(List<PersistenceXmlDescriptorBuildItem> descriptors) {
        ...
        List<String> notSetPersistenceUnitsDBKinds = descriptors.stream()
                ...
                .map(IAMShieldProcessor::getDatasourceNameFromPersistenceXml)
    ...
    private boolean missingDbKind(String datasourceName) {
        String key = NS_IAMSHIELD_PREFIX.concat(DatabaseOptions.Datasources.getNamedKey(DatabaseOptions.DB, datasourceName).orElseThrow());
    ...
    void configurePersistenceUnits(HibernateOrmConfig config,
        ...
        for (PersistenceXmlDescriptorBuildItem item : descriptors) {
            ...
            if (DEFAULT_PERSISTENCE_UNIT.equals(descriptor.getName())) {
                ...
                runtimeConfigured.produce(new HibernateOrmIntegrationRuntimeConfiguredBuildItem("iamshield", defaultUnitDescriptor.getName())
                ...
                runtimeConfigured.produce(new HibernateOrmIntegrationRuntimeConfiguredBuildItem("iamshield", descriptor.getName())
    ...
    private void configureDefaultPersistenceUnitEntities(ParsedPersistenceXmlDescriptor descriptor, CombinedIndexBuildItem indexBuildItem,
        ...
        for (AnnotationInstance annotation : annotations) {
            ...
            if (!userManagedEntities.contains(targetName)
                    && (!targetName.startsWith("org.iamshield") || targetName.startsWith("org.iamshield.testsuite"))) {
    ...
    @Produce(IAMShieldSessionFactoryPreInitBuildItem.class)
    void configureIAMShieldSessionFactory(IAMShieldRecorder recorder, List<PersistenceXmlDescriptorBuildItem> descriptors) {
    ...
    private List<ClasspathThemeProviderFactory.ThemesRepresentation> loadThemesFromClassPath() {
        try {
            ...
            Enumeration<URL> resources = Thread.currentThread().getContextClassLoader().getResources(IAMSHIELD_THEMES_JSON);
    ...
    void configureConfigSources(BuildProducer<StaticInitConfigBuilderBuildItem> configSources) {
        configSources.produce(new StaticInitConfigBuilderBuildItem(IAMShieldConfigSourceProvider.class.getName()));
    }
    ...
    void prepareTestEnvironment(BuildProducer< StaticInitConfigBuilderBuildItem> configSources, DevServicesDatasourceResultBuildItem dbConfig) {
        configSources.produce(new StaticInitConfigBuilderBuildItem("org.iamshield.quarkus.runtime.configuration.test.TestKeycloakConfigSourceProvider"));
    ...
    void index(BuildProducer<IndexDependencyBuildItem> indexDependencyBuildItemBuildProducer) {
        ...
        indexDependencyBuildItemBuildProducer.produce(new IndexDependencyBuildItem("org.iamshield", "iamshield-services"));
    }

    @BuildStep
    @Consume(CheckJdbcBuildStep.class)
    void indexJpaStore(BuildProducer<IndexDependencyBuildItem> indexDependencyBuildItemBuildProducer) {
        indexDependencyBuildItemBuildProducer.produce(new IndexDependencyBuildItem("org.iamshield", "iamshield-model-jpa"));
    }
    ...
    private static void disableClusterHealthCheck(BuildProducer<BuildTimeConditionBuildItem> removeBeans, CombinedIndexBuildItem index) {
        ClassInfo clusterHealth = index.getIndex().getClassByName(DotName.createSimple(IAMShieldClusterReadyHealthCheck.class));
    ...
    private static void disableReadyHealthCheck(BuildProducer<BuildTimeConditionBuildItem> removeBeans, CombinedIndexBuildItem index) {
        ClassInfo disabledBean = index.getIndex().getClassByName(DotName.createSimple(IAMShieldReadyHealthCheck.class.getName()));
    ...
    void configureResteasy(CombinedIndexBuildItem index,
        ...
        chainCustomizers.add(new IAMShieldHandlerChainCustomizer());

        if (Configuration.isTrue(TracingOptions.TRACING_ENABLED)) {
            chainCustomizers.add(new IAMShieldTracingCustomizer());
            ...
            preExceptionMapperHandlerBuildItemBuildProducer
                    .produce(new PreExceptionMapperHandlerBuildItem(new IAMShieldTracingCustomizer.EndHandler()));
        }
    ...
    @BuildStep
    @Record(ExecutionTime.STATIC_INIT)
    void setCryptoProvider(IAMShieldRecorder recorder) {
        FipsMode fipsMode = getOptionalValue(NS_IAMSHIELD_PREFIX + SecurityOptions.FIPS_MODE.getKey())
    ...
    void configureDevMode(BuildProducer<HotDeploymentWatchedFileBuildItem> hotFiles) {
        hotFiles.produce(new HotDeploymentWatchedFileBuildItem("META-INF/iamshield.conf"));
    }

    @Record(ExecutionTime.STATIC_INIT)
    @BuildStep
    void configureProtoStreamSchemas(IAMShieldRecorder recorder) {
    ...
    private Map<String, ProviderFactory<?>> loadDeployedScriptProviders(ClassLoader classLoader, Spi spi) {
        ...
        if (supportsDeployeableScripts(spi)) {
            try {
                Enumeration<URL> descriptorsUrls = classLoader.getResources(IAMSHIELD_SCRIPTS_JSON_PATH);
    ...
    private List<ScriptProviderDescriptor> getScriptProviderDescriptorsFromJarFile(URL url) throws IOException {
        ...
        try (JarFile jarFile = new JarFile(file.substring("file:".length(), file.indexOf(JAR_FILE_SEPARATOR)))) {
            JarEntry descriptorEntry = jarFile.getJarEntry(IAMSHIELD_SCRIPTS_JSON_PATH);
    ...
    private boolean isInternal(ProviderFactory<?> factory) {
        ...
        return packageName.startsWith("org.iamshield") && !packageName.startsWith("org.iamshield.examples");
    }
    ...
    private void checkProviders(Spi spi,
        ...
        if (provider != null) {
            ...
            String defaultProvider = DefaultIAMShieldSessionFactory.resolveDefaultProvider(factories, spi);
