...
<dependencies>
    <dependency>
        ...
        <artifactId>iamshield-quarkus-server</artifactId>
    </dependency>
</dependencies>
```
...
the `deployment` and `runtime` modules is the `org.iamshield.quarkus.runtime.KeycloakRecorder` class. The `KeycloakRecorder` holds
...
Quarkus will run the `org.iamshield.quarkus.deployment.KeycloakProcessor.configureProviders` build step whenever you are (re)augmenting the server. The main outcome from this method
is to invoke the `org.iamshield.quarkus.runtime.KeycloakRecorder.configSessionFactory` so that the code from this method is executed at
...
The `server` module holds the Keycloak Server itself. It is basically a regular Quarkus application using the `iamshield-quarkus-server` extension. If you look at the
...
is the `org.iamshield.quarkus.runtime.KeycloakMain` class from the `runtime` module.
...
* Changes are only automatically reflected at runtime if you are changing resources from the `deployment`, `runtime`, and `server` modules. Other modules, such as `iamshield-services` still rely on Hot Swap in Java debuggers to reload classes.
...
```
cp services/target/iamshield-services-${KC_VERSION}.jar ${KC_HOME_DIR}/lib/lib/main/org.iamshield.iamshield-services-${KC_VERSION}.jar
```
...
Before contributing changes, make to read the main [Keycloak Contributing Guide](https://github.com/iamshield/iamshield/blob/main/CONTRIBUTING.md).
...
* [Configuration Guide](https://www.iamshield.org/server/configuration)
